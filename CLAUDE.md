...
- **Unit + integration tests:** `for t in producer/*.test.mjs; do node "$t"; done` — covers the
  ...
  **trend gate** and the **recent-stop-out cooldown** — `gradePickClose`/`recentStopCooldown`),
  ...
  refresh events, `agentic-ledger` decision grading), and a full `build-data.mjs` fixture run
  (empty-bars guard, quotes carry-forward, no-picks log guard, social-sidecar reuse, day-move alert). CI runs these on every
