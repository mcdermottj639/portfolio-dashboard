...
- **Versioning:** any change to `index.html`/`sw.js` → bump **both** `APP_VERSION` (in `index.html`
  `boot()`) and `CACHE_VERSION` (in `sw.js`) together. Currently around **v127** (`pf-v127`).
...
- **Unit + integration tests:** `for t in producer/*.test.mjs; do node "$t"; done` — covers the
  ...
  **trend gate** and the **recent-stop-out cooldown** — `gradePickClose`/`recentStopCooldown`),
  ...
  refresh events, `agentic-ledger` decision grading + `activityFromDecisions`, **`maindecisions`** (the self-directed log: the ET-fill grouping incl. the previous-evening boundary and the placed-vs-filled distinction, same-day clips collapsing to one share-weighted leg, the DRIP/cancelled filters, `spyAt` stamped from that day's close and `null` rather than guessed when absent, both bar shapes + the interpolated/live skips, the snapshot merge — out-of-window carry-forward, in-window sweep of a day the broker no longer reports, the owner-overlay annotation, the cap — and an end-to-end pass proving the derived log grades through `agentic-ledger.gradeDecisions` unchanged with no sleeve attribution), **`agentic-pending`** ticket
...
- **Picks (the "🎯 Plan" tab — renamed from "Picks" in v56):** **sortable + sector-filterable** scored candidates table incl. a
  ...
  **🧾 Rebalance Log (v127)** sits directly under Track Record, and the pairing is the point: Track
  Record grades the *picks screen*, this grades *what the owner actually did*. One row per ET trading
  day on ••••0741 — the trades, the dollar-weighted contribution since the fill, the same window's
  **alpha vs SPY**, and an ahead/behind/open verdict, identical to the agentic account's log so the two
  books can be judged on the same terms. Rows are DERIVED from filled orders rather than appended by an
  executor (there is none on this side — see `maindecisions.mjs`), and the card says so. The most recent
  12 days show; older ones sit behind a `<details>` expander so it can't grow unbounded. Deliberately
  **without** the agentic card's research-sleeve strip: there are no sleeves on this discretionary book,
  and rendering them would be one mandate's rulebook leaking onto the other's surface.
