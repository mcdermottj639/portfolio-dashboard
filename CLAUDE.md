...
| `producer/agentic-deploy.mjs` | **Pure deployment/rebalance planner (v93, unit-tested).** `planDeployment({target, positions, cash, quotes, earnings, washMap})` → a ready-to-confirm ticket, enforcing the execution discipline that was previously only prose: **earnings blackout** (never deploy NEW money into a name ≤7d from its report — wait for the print), **gap-through-entry re-verify** (a name at/below its target stop = broken → defer; below its planned entry zone = thesis in question → defer, the GOOGL-after-earnings case), **wash-sale** (reads `recentLosses`), and **cash-flow-first / T+1** sequencing. Buys move each underweight name only toward its (already cluster/vol-capped) target, so honoring the target respects the risk caps. Mirrors the consumer's Agentic-card badges so card + ticket agree. |
...
- **Unit + integration tests:** `for t in producer/*.test.mjs; do node "$t"; done` — covers the
  ...
  **trend gate** and the **recent-stop-out cooldown** — `gradePickClose`/`recentStopCooldown`),
  ...
  `agentic-deploy` earnings-blackout/gap-reverify/wash-sale planner, `agentic-triggers` deposit +
  refresh events, `agentic-ledger` decision grading), the **flow scorers** (`flow.mjs` — revision
  direction-over-level, the insider open-market filter + buy/sell asymmetry regression, surprise
  clamping, composite renormalization + the abstain-below-2-components rule), and a full `build-data.mjs` fixture run
  (empty-bars guard, quotes carry-forward, no-picks log guard, social-sidecar reuse, day-move alert,
  **flow sidecar → `data.flow` + per-symbol carry-forward**). CI runs these on every
...
- **Flow & Positioning data (v95, producer-side only so far):** `data.flow` now carries per-symbol analyst
  revision momentum, insider Form 4 clusters and earnings-surprise drift for the held book (see
  `flow.mjs` above). **Nothing renders it yet and nothing consumes it in the target** — it is accumulating
  through a burn-in so the signal can be judged on real data before it earns weight. The consumer card +
  Analyze chip and the research sleeve are the remaining phases in `PROPOSAL-flow-signals.md`.
