...
proposing trades the governor would refuse. **The cron is UNCHANGED** (`12 11 * * 1`): the gate prints
NOT_DUE on the off weeks, which costs one cheap gate run and needs no Routine edit.

**One thing still needs the owner:** the Routine prompt must pass `args.momentum` to skip that agent.
Without it the workflow logs that it is falling back to the momentum AGENT and runs as before — it
degrades, it does not break. Paste-ready text is in `SCHEDULING.md` §5.
