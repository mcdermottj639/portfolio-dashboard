...
`agentic-research` is the NUMBER of agents, not which model each runs. Three changes, no change to what
the pipeline decides:

- **momentum is CODE now** (`producer/momentum.mjs`, passed in as `args.momentum`, the `args.flow`
...
- **verify is BATCHED**: it was one agent per finalist and measured the largest phase (~2.7× the whole
  ...
  `FINALIST_CAP` 16 → 10, `CHALLENGER_SLOTS` 5 → 3 (the quota shrank WITH the cut, it was not dropped).

**Cadence is now FORTNIGHTLY** (`agentic-due.mjs`), because the churn governor already runs on a 14-day
clock (`MIN_HOLD_DAYS`/`REENTRY_COOLDOWN_DAYS` 14) — a target refreshed every 7 days spent half its life
proposing trades the governor would refuse. **The cron is UNCHANGED** (live: `12 23 * * 0`): the gate prints
NOT_DUE on the off weeks, which costs one cheap gate run and needs no Routine edit.

**Routine prompt APPLIED the same day** (`update_trigger` on `trig_0114s3r8yBA7rQXLsBY7MG1y`): step 3b runs
`node producer/momentum.mjs --symbols <universe> --out producer/raw/momentum.json` (a CLI added for it —
the first draft had the agent hand-edit an inline `node -e` with a `/*the universe tickers*/` placeholder)
and passes the file as `args.momentum`. If it fails the workflow logs the fallback to the momentum AGENT and
runs as before. **The live cron is `12 23 * * 0` (Sunday 23:12 UTC), not the `12 11 * * 1` older notes
below quote** — it was moved 2026-09-15. A Sunday fire is the LAST day of its Monday-anchored fortnight, so
the first run after a lapse can land one week after the previous one (the new fortnight starts the next
day); from then on it settles to every other Sunday. Harmless — it errs toward running.

Verification: full `producer/*.test.mjs` suite (34 suites green), including a new `momentum.test.mjs`
that pins the stale-series abstention, and `agentic-correctness.test.mjs`'s end-to-end workflow
simulation driven through the new batched-verdict contract.
...
3. Agent makes the Robinhood MCP calls and `Write`s raw JSON into `producer/raw/` (gitignored). This
   ...
   **On FETCH_ALL the historicals + bench-quotes symbol lists are no longer hand-computed (v141):**
   the agent runs **`node producer/hist-plan.mjs`** and fetches exactly the batches it prints —
   `hist-day-tail-<n>.json` (8 symbols, ~7 bars each) · `hist-day-full-<n>.json` (≤3 symbols, YTD,
   capped 30/run) · `hist-month-<n>.json` (≤3, capped 15/run) · `quotes-bench-<n>.json`. That
   replaces the "top 15 holdings by value + market symbols" rotation, which is why 142 of 191 daily
   series were stale on 2026-09-08.
4. Agent runs **`node producer/run.mjs "<label>"`** — the single deterministic tail: optional AV
   fetch → optional **ext-fund fetch** (Finnhub/FMP supplementary fundamentals — since v141 over the
   whole **Analyze bench**, Finnhub daily and FMP on a 45/day least-recently-refreshed rotation) →
   optional **flow fetch**
...
6. **FETCH_ALL only, after publish, ~weekly (best-effort):** `agentic-due.mjs` gates the agentic-account
   ...
   owner a rebalance proposal** (places nothing — alert, then the owner's confirm). Fault-isolated like step 5;
...
> **MANDATE A (2026-09-08) touched three of these four.** The **weekly research** Routine's prompt is the
...
> **producer** needs no prompt change at all: deposit tranching reuses `cashIdleDays`, which build-data
> already stamps. Nothing here changes what is FETCHED.
...
**THERE ARE TWO INDEPENDENT GATES, and a prompt is editable only when BOTH pass (corrected 2026-09-08 —
this entry has now been wrong in both directions, so TEST the edit rather than reasoning about it):**
1. **`created_via` must be `meta_mcp`** (agent-created). **`http_api` = made in the claude.ai Routine UI =
   `update_trigger` REFUSES it** (*"Agents can only update routines they created"*). Seven of the eight
   Routines here are `meta_mcp`; the single `http_api` one is **"Portfolio dashboard refresh"** — the
   hourly producer, i.e. the most important of them — so its prompt is the owner's to paste in the UI.
2. **The Routine must not fire into a session that isn't yours.** A Routine bound to an interactive
...
made on 2026-09-08 (Mandate A) and refused. Schedule, name and enabled state still change freely on both.
**The durable lesson is the one that survived every revision: put anything load-bearing in CODE, never in
...
- **The executor Routine's prompt IS editable — that claim was wrong, and it was wrong for weeks.** This
  bullet used to say it "genuinely cannot be edited" because the Routine was bound to a persistent session.
  The trigger was updated on 2026-08-31 (the enabled flag), and a **fresh-session-per-fire** replacement
  with a corrected prompt was created 2026-09-02 (disabled until Robinhood is attached to it in the UI), so
  the prompt is no longer frozen by the binding. Its prompt still says only "`makeDecision`,
  with `spyAt`" — fix it there, and **keep every code-level guard anyway**: the exec gate writes `target`
  into `raw/agentic-plan.json`, `AGENTIC.md` step 3e carries a blockquote that overrides the prompt, and
  `makeDecision` **logs a loud warning** when it is called with trades but no `target` or no positive
  `spyAt`. The general rule survives the correction and is the more useful half: **prefer a code guard to
  prompt wording for anything load-bearing**, because a prompt is server-side and drifts out of sight of
  this repo. What genuinely CANNOT be set from a session is a Routine's **connectors and `allowed_tools`**
  — those are Routine-UI only (see the 2026-09-02 gotcha).
...
| `producer/build-data.mjs` | Assembles + encrypts `data.json`; **merges `data.hist` at the BAR level through `histbars.mergeHist`** (v141 — the old per-symbol whole-array spread could only replace or carry forward, which is what made a wide fresh bench unaffordable) and then **compacts it through `histbars.compactHist`** (v139 — one call site, right after that merge, halving the published file); emits **`data.vix`** (v121 — `{v,asOf}` parsed from the already-recorded `INDEX_DATA` VIX; additive, nothing keys on it, carried forward, absent when unparseable) so the producer-side deploy planner can pace by regime without re-implementing the AV response parsing; **carry-forward overlay** (decrypts prior snapshot once, overlays fresh on **quotes**/hist/recorded/picks/options/realized/**notes** — quotes carry forward per-symbol since v88 so a transiently unquotable name keeps its last price instead of dropping to $0; a fresh-but-EMPTY bars array also no longer clobbers carried-forward hist). Also maintains **`data.picks.history`** — when a fresh scan replaces the prior picks (new date), the outgoing picks (entry/TP1/TP2/stop) are archived (cap 40) so the consumer can grade the Track Record. Also grades that archived history through **`pickgrade.mjs`** into **`data.picks.grades`** (`{asOf,horizonDays,episodes[],stats,universe[]}`) — frozen outcomes plus the self-draining list of symbols whose prices the fetch must keep alive — and **warns loudly, naming them**, whenever an episode cannot be graded, because the only fix is upstream in the fetch. Maintains **`data.options.ivHistory`** too — appends each run's `ivObserved` (one point/UTC-day, cap ~260) and derives **`data.options.ivRank`** (where today's IV sits in its trailing range), decorating each position/idea with `ivRank`. Optional `producer/notes.json` (a string or `{risk:"…"}`) → `data.notes` for owner editorial that renders in the Risk card without baking prose into `index.html`. **COMPANY_OVERVIEW accumulation guard:** the free AV tier (25/day + burst throttle) only covers a rotating subset of holdings per run; this run's 11-field Robinhood-synth overview (P/E·MktCap·DivYld only) is NOT allowed to clobber a carried-forward AV-rich overview (one with `ForwardPE`/`EPS`/`QuarterlyRevenueGrowthYOY`) for the same symbol — genuine AV refreshes (also rich) still win — so Fwd P/E / Rev Growth / EPS coverage **accumulates across days** instead of flickering blank for whichever names missed today's cap. Also emits **`data.agentic`** (v67) = the agentic account's `{asOf,cash,buyingPower,equity,positions[]}` from optional `agentic-portfolio.json`/`agentic-positions.json` (priced from `data.quotes`, carry-forward) — the actual holdings the **Agentic Portfolio** card renders its target against. Maintains **`data.agentic.equityHistory`** too (v72 — same shape as `ivHistory`: one `{t,equity,cumFlow}` point/UTC-day, latest wins, cap ~260) — the account's **real** equity recorded **forward** (Robinhood has no account-equity-history endpoint, so it can't be backfilled); the Portfolio Performance chart overlays it. **Deposit-adjustment (v92):** this is a self-funded account the owner adds to over time and Robinhood exposes no transfers feed, so a deposit inflates equity and would read as a fake return (a $1k→$3.5k funding jump = a bogus +250% — the bug that prompted this). build-data now **infers the net external cash flow** each run — `flow ≈ ΔEquity − Σ(priorQty × price move)` (a deposit lands in cash without a matching position change; internal buys/sells net to ~0; qty-unchanged gaps are exact) — past a noise floor (`max($40, 8% of prior equity)`), and stores it as a running **`cumFlow`** on each point. The consumer computes a **time-weighted, deposit-immune** return from `cumFlow` deltas (see `renderPerformance`), with an implausible-jump fallback (>20% in one step → treated as an un-annotated legacy deposit) so already-recorded points before this field are corrected too. Also maintains **`data.agentic.recentLosses`** (v91 — the wash-sale ledger; rolling 31-day retention, read by the Agentic card to **block + flag** rebuying any name inside the 30-day window). **v98 sources it from REAL closing trades** — `producer/raw/agentic-trades.json` (`get_pnl_trade_history`, span `ytd`) via `lossesFromTrades` — and rebuilds it wholesale on each fetch, stamping `data.agentic.lossSource='trades'`. **v105 makes it CROSS-ACCOUNT:** the ledger also merges the **self-directed ••••0741 book's** realized losses (`producer/raw/main-trades.json`, `get_pnl_trade_history` span `3month`, every run), each entry tagged `account:'main'|'agentic'`, because the IRS wash window is per TAXPAYER — on 2026-07-29 the owner sold 35 NVDA at −$431.76 in ••••0741 and the agentic executor rebought NVDA on 2026-08-11 through a ledger that only read ••••3900's (empty) trade history, partially disallowing the loss. Each account's portion rebuilds when its file is present and carries forward (expiring) when not; there is NO inference for the main portion, ever. The original **inference** (diff prior→fresh positions; a holding reduced/exited while underwater = a loss dated today) survives only as the Railway fallback and **never wins over real trades, nor layers on top of a trades-sourced ledger**: it is unsound, because any run whose agentic fetch returned the *wrong account's* positions makes the next correct fetch look like a mass liquidation. That is what happened — five losses booked 2026-08-03 (LLY/NVDA/TSM/CIFR/IREN) for an account with **no closing trades that week** that had never held three of those names, and NVDA was wash-sale blocked out of a real buy for 30 days off one of them. Also assembles **`data.realized`** per account (v98, via `realizedpnl.mjs`) from `realized-main{,-opt}.json` / `realized-agentic.json` (`get_realized_pnl`, per asset class): `{year,asOf,source:'robinhood',approx:false,accounts:{main,agentic},equity,options,total,premiumYTD}` — top-level fields are **all-account sums** (so an older cached consumer shows a correct combined figure), `accounts` carries the split. Precedence: fresh broker fetch → committed `producer/realized.json` (gitignored owner override / Railway fallback) → prior snapshot; the options-book `realizedYTD` override is skipped when `source==='robinhood'` so it can't desync `accounts` from the totals. **v93:** also grades **`data.agentic.decisions`** from committed `agentic-decisions.json` (via `agentic-ledger.mjs`, vs live quotes + SPY) and writes **`producer/raw/agentic-triggers.json`** (via `agentic-triggers.mjs` — deploy-cash + research-refresh events). **2026-08-30:** also emits **`data.realized.predictionMarket`** `{asOf,year,ytd,count,trades[]}` (via `mergeEventTrades` over `main-trades.json`) — kept deliberately OUT of `realized.total` and out of `accounts`, because those are broker-reported per-account figures and folding a snapshot-accumulated ledger into them would desync the split from its own total (the same failure the options-override guard prevents). The consumer renders it as its own strip. |
| `producer/histbars.mjs` | **Bar compaction (v139, pure + unit-tested).** `compactBar`/`compactSeries`/`compactHist` shrink `data.hist` to the shape every reader ALREADY coalesces to — `{t,c,h,l,v}` plus `interpolated`/`live` only when true. Called from ONE site in `build-data.mjs`, immediately AFTER the carry-forward merge so it covers fresh and carried-forward series alike. **Measured on the live 2026-09-08 snapshot: hist was 4.88MB of 5.33MB (91.5%) at 159 bytes/bar over 29,202 bars; the published file went 7.13MB → 3.48MB (51%), and projected git growth 2.72 → 1.32 GB/month.** Two fields were dropped because grep proves NOTHING reads them: **`open_price`** (the chart analyzer derives a candle's open from the PRIOR CLOSE, so it was dead weight from day one) and **`session`** (every "session" hit in the repo is sessionStorage or a session-bound Routine); prices also stop being zero-padded strings (`"230.360000"` is 12 bytes for a number needing 6) and `interpolated:false` stops being written at all. **Four invariants.** (a) **Idempotent** — build-data re-runs it over its own carried-forward output every hour, so `compact(compact(x)) === compact(x)` or the snapshot would churn its own bytes and defeat the point. (b) **Array length is preserved exactly; a bar is shrunk, never dropped, not even a junk one** — `betaFromHist`/`corrGroups` align two series by TAIL INDEX (`slice(-n)`), not by date, so removing one bad bar from one symbol shifts it a day out of phase and silently corrupts beta and correlation (the v111 lesson: a uniformly stale series merely lags, a mis-aligned one is WRONG). (c) **The timestamp is copied VERBATIM**, never reformatted — readers slice it, date-parse it and compare it to ET dates computed elsewhere, and shortening it would save ~6% more for the price of a date-comparison regression. (d) **A field is omitted only where its reader's fallback yields the same value** — `h`/`l` fall back to `c` and `v` to `0`, which is exactly why those three and no others are droppable. A bar with no finite close passes through untouched. **This is safe only because of the documented Railway-vs-Claude bar-shape coalescing** (`b.begins_at||b.t`, `b.close_price ?? b.c`, `b.volume ?? b.v`), which is now load-bearing twice over; the consumer has in fact been mixing this exact compact shape into raw series since v111, because `spliceLiveBars` appends `{t,c,live:true}`. **`mergeBars`/`mergeHist` (v141) — the TAIL MERGE, the other half of the same ownership of the bar shape.** `build-data.mjs` merged historicals as a per-symbol WHOLE-ARRAY replace, so a symbol was either re-fetched in full or untouched — there was no third option, which is exactly why keeping a wide bench fresh looked unaffordable (~153 YTD bars x ~230 names, every day, into an encrypted blob committed ~13x/day). Now a **7-bar tail (~0.5KB/symbol)** glues onto the series already held. **The rule is a UNION and is deliberately STRICTER than what it replaced:** bars are keyed by DATE (`(t||begins_at).slice(0,10)`), fresh wins a collision, and a bar that exists only in PRIOR is always kept — because a whole-array replace silently TRUNCATES a series whenever a fetch comes back short (a bad `start_time`, a broker hiccup, a partial page), and a truncated series is indistinguishable downstream from a name that simply has less history. Interpolated placeholders are kept exactly as before (readers filter them); a `live:true` bar is dropped defensively, since an intraday print is not a close. Idempotent like compaction, and it runs **BEFORE** `compactHist`, so all four compaction invariants still see every bar exactly once. The pairing that makes a tail SAFE lives in `hist-plan.mjs`: its tail window must reach back FURTHER than the oldest bar a tail is allowed to bridge, or the merge would faithfully preserve a hole. |
| `producer/emit.mjs` | AES-GCM encrypt/decrypt (`encryptEnvelope`/`decryptEnvelope`). |
| `producer/equityseries.mjs` | **Recorded account-equity history (v119, pure + unit-tested)** — the shared basis of BOTH accounts' real YTD. `appendEquityPoint` (one point per UTC day, latest wins, cap 260) · `inferFlow` (deposits/withdrawals: `flow ≈ ΔEquity − Σ(priorQty × price move) − Δoptions_value`, past a noise floor) · `flowThreshold`. Extracted verbatim out of `build-data.mjs`'s agentic block so the **self-directed** account could record the same way — the two accounts' YTD figures can now differ only in their inputs, never in their math. Two things it knows that a caller must not re-decide: **equity is the BROKERAGE book** — `equity_value + options_value + cash`, via **`brokerageEquity`** (v143; it was `total_value` until 2026-09-10, when that was measured to INCLUDE the derivatives sleeves — the `+ cash` term is the v116 lesson intact, since `equity_value` alone overstates a margin book by the whole loan, and deriving from components makes the basis independent of whether the broker folds a sleeve into its headline number). `brokerageEquity` also returns `residual` — the invariant that `total_value` is fully explained by that book plus the sleeves in `EXTERNAL_SLEEVE_KEYS`; a non-zero residual means an unmodelled asset bucket and is **pushed**, not just logged. The perimeter move is booked once as a `basisShift` flow (never subject to the noise floor, stamped on the point, and it survives same-day re-runs — the producer replaces the day's point ~13x/day). **`absent !== zero` bites here:** every component must be PRESENT or the whole thing falls back to `total_value`, because the **Railway** producer writes no `options_value` and reading that as 0 would silently drop the options mark out of the basis on the one producer nobody watches — so a Railway run keeps the pre-v143 basis and says so (a known, documented divergence — see `RAILWAY.md`; don't alternate producers mid-week on this account or the series mixes two bases). And the **options-value term is P&L, not a transfer** (a short call's mark moving $300 is not a $300 deposit; only the self-directed book needs it). Noise floor = `max($40, min(8% of prior equity, $750))` — the **$750 cap is new**: at a flat 8% a $19k book would need a $1,520 move before a transfer registered, which is larger than most real deposits. **`derivativesRealized` (2026-08-30) is the third term:** prediction-market (event-contract) and futures settlements pay into `cash` from sleeves that sit OUTSIDE the recorded brokerage book, so a winning bet is shaped exactly like a deposit and a losing one exactly like a withdrawal. It sums the realized P&L of those trades for THIS step only (`since` = the prior snapshot's `asOf`, exclusive; `until` = this one's) and `inferFlow`/`appendEquityPoint` subtract it as `extraPnl`. The discriminator is a **blank `symbol`** (shared with `realizedpnl.isDerivativeTrade`) — see the gotcha. No `since` ⇒ it returns 0 and abstains. **`inferCashFlow` REPLACED the price-move sum as the PRIMARY (2026-08-30):** `flow = Δcash + Σ(Δqty × px) − extraPnl`. External money must land in CASH, and an internal trade moves cash and position value in opposite directions by the same amount, so the two terms cancel a trade exactly and leave a transfer standing. Prices enter ONLY through the Δqty term — zero on almost every run — which makes it immune to the stale-quote skew that produced ±$1,000/day of phantom flow (see the gotcha), and EXACT when nothing traded. Returns `null` (⇒ caller falls back to `inferFlow`) when cash is missing on either side or a traded symbol can't be priced. `inferFlow` is now the documented legacy fallback for pre-v119 snapshots. |
| `producer/drawdown.mjs` | **Book-level drawdown circuit breaker (v121, pure + unit-tested).** The missing PORTFOLIO-level risk control — every other guard in this system is name-scoped (per-name stops, entry bands, min-hold), so nothing said "the BOOK is down X% from its high, stop putting new money in". `bookDrawdown(equityHistory)` → `{dd, level:'ok'|'soft'|'hard', peakT, minDdSincePeak}`; `twrSeries` is the shared index. **Two invariants:** (a) it runs on a **time-weighted, deposit-adjusted** index, never raw equity — a $500 deposit into a $850 book that fell from $1,000 makes RAW equity a new all-time high and would cancel the breaker at exactly the worst moment (and a withdrawal would fake a drawdown); (b) it is **memoryless by construction** — the executor may commit only three files and never `data.json`, so a persisted "tripped" flag has nowhere legal to live; hysteresis instead comes from `minDdSincePeak`, which the series itself records. **MANDATE A (2026-09-08) — THE TRIGGER IS NOW RELATIVE TO SPY.** v121's absolute −8%/−12% was a tail guard on a ~0.65-beta book (15% defensive floor + 5% gold + 22.5% index) and never fired. Mandate A removes the ballast, beta goes to ~1.0-1.1, and at THAT beta an absolute −8% fires in an ordinary correction while the hard tier would **raise 20% cash into the hole** — selling the bottom of a routine pullback and sitting out the rebound, i.e. market timing wearing a risk-control costume, which the mandate forbids. The question is not "is the book down?" but **"is the book down MORE THAN THE MARKET?"** — a market-wide fall is beta a beat-SPY book is paid to carry; only a fall the market did NOT share means the model is failing. Thresholds `AG_DD_REL_SOFT` **−5pp** / `AG_DD_REL_HARD` **−8pp** / `AG_DD_REL_RESUME` −3pp, measured from the book's own peak date against SPY over the identical window, plus **`AG_DD_ABS_BACKSTOP` −20%** which binds regardless (a purely relative breaker would let the book fall indefinitely so long as SPY fell with it; −20% is deep enough that no ordinary correction reaches it, so it cannot smuggle the old behaviour back). **A THIRD INVARIANT came with it: the bench must be FRESH or it is REFUSED** — `data.hist.day` goes stale PER SYMBOL, and a stale SPY series reads as "the market did not move", which turns every market-wide fall into an apparent idiosyncratic one and trips the breaker on exactly the case it exists to ignore; past `DD_BENCH_STALE_DAYS` (5) it falls back to `basis:'absolute-only'`. `agentic-exec-gate.mjs` passes `data.hist.day.SPY`; the result carries `dd`/`ddBench`/`relDd`/`basis`. **`build-data.mjs` MUST pass the same bench (verify catch, 2026-09-08)** — it computes the `data.agentic.drawdown` the card, the hand-off and `alerts.mjs`'s push all read, and the first build passed the bench only to the gate: the snapshot would have carried an absolute-only read (trips at −20%) while the executor tripped at −5pp relative, so the breaker would have stopped buying with **nothing on the phone and no push saying so** — the "guard that fires silently" failure, reached through the one seam the site's own comment promised could not diverge. `build-data.test.mjs` now pins it via `benchStale:true`, a flag that can only be set when a bench actually reached the function. **Fails OPEN** below `DD_MIN_POINTS` (5) — the equity series cannot be backfilled, so a young account is thin by definition and a breaker that stopped on thin data would freeze a new book forever. Consumed by `agentic-deploy.mjs` via `planDeployment({drawdown})`, computed by `agentic-exec-gate.mjs` from the committed snapshot. |
| `producer/realizedpnl.mjs` | **Broker realized-P&L normalizers (v98, pure + unit-tested).** `sumRealized` (one `get_realized_pnl` response → total/trades; null "transfer-only" buckets are **n/a, not $0**), `accountRealized` (equity + option classes → one account block; a missing class stays `null`, not 0), `buildRealized` (per-account blocks → `data.realized` with all-account totals), `lossesFromTrades` (`get_pnl_trade_history` → the dated realized **losses** feeding the wash-sale ledger; takes an optional `account` tag (v105) so the cross-account merge knows which taxable book booked each loss). **`WASH_MIN_LOSS` — a $25 DE-MINIMIS FLOOR (owner-set 2026-09-03).** A loss under it never enters the ledger, so it blocks nothing: this ledger is a **pre-trade avoidance tool, not a tax record** (Robinhood reports within-account wash sales on the 1099-B regardless), and ignoring a trivial one costs a timing difference on pennies — a disallowed loss is not lost, it moves to the replacement shares' basis — while honouring one costs a real 30-day block on a real allocation. Live: a **$1.01** VTI loss was deferring a $29.25 VTI target buy to Oct 2, and VTI is the parking vehicle, so its own park/release round trips manufacture exactly these pennies (−$1.01, −$0.24) and would have kept the target's own 5% ballast permanently gated. Two load-bearing details: the floor tests the symbol's **TOTAL loss for the day**, not each fill (five −$20 clips are a −$100 loss and must still block), while the stored `realized` stays the largest SINGLE loss so only what is FILTERED changed; and it **cannot reach the Railway inference fallback**, whose entries carry no `realized` at all — unknown magnitude must fail safe and keep blocking. Applied at this one site so every consumer (planner, exec gate, both cards) inherits it; the carry-forward branch can still hold pre-floor dust for up to 31d if the trades file is ever missing, which is bounded and self-healing because the fetch is every-run. `asset_classes` is **required** by the endpoint — omitting it errors `un-specified asset class`. **`isDerivativeTrade` + `mergeEventTrades` (2026-08-30)** own the PREDICTION-MARKET line item: `get_realized_pnl` is per asset class (equity/option) and never reports event contracts, so a settled bet appears in no realized figure anywhere. `mergeEventTrades` accumulates the settlement ledger **in the snapshot** (raw/ is wiped every run and the fetch is a rolling 3-month window — the `ivHistory` pattern), de-duped on `timestamp|qty|realized` because the feed re-delivers the same settlement on every run for three months (counting it per-run would turn a $1,008 win into five figures by November) — **and the key is built from the ROUNDED amount (v143)**, because Robinhood returns sub-cent precision on some settlements (`realized_gain: "127.542"`) and keying on the raw value while storing the rounded one made a prior entry and its own source row hash differently: the live 2026-06-26 win was counted twice and prediction-market YTD read **$223.05 against a true $95.51**. Keying on the rounded value also retires the damage — the duplicated rows collapse on the next run, so no migration was needed, retained by TIME not count (`EVENT_RETAIN_YEARS` 3; `EVENT_CAP` is a runaway backstop only — the `maindecisions.mjs` lesson), and a malformed/absent payload returns the prior ledger **untouched** rather than rebuilding from empty and silently zeroing the year. |
...
| `producer/pickgrade.mjs` | **Daily-Picks track record — grading, frozen outcomes, and the fetch universe that keeps it honest (2026-09-09, pure + unit-tested).** The card used to grade every archived pick on the PHONE, walking `data.hist.day` on each load — and that series goes **stale per symbol**, so a pick's closes freeze within days of the scan that named it. Measured on the live 2026-09-08 snapshot: **12 of 44 episodes were graded over ZERO bars**, most over fewer than ten, and the card reported **0% hit rate · 0W·2L · 42 open · +2.1% avg**. Regraded against the real closes, the same 44 episodes are **9W·3L — a 75% hit rate**. The 0% was a statement about the price history, not about the picks. **Three rules, each a lesson already paid for elsewhere in this repo.** (a) **A series that does not reach the measurement date is `UNMEASURED`, never `OPEN`** — "open" claims we watched the name and it touched neither level, which is false when the series stopped three days in; the same abstention `agentic-ledger`'s `markFromBars` makes, for the same reason (a stale series stops at whatever price it stopped at, which is evidence of nothing). It carries **no return** either. The censoring was **directional**, which is why the damage was one-sided: an early dip through a stop lands inside the few bars we do have, while a target hit weeks later never does — so the measurable window could only ever produce losses and "open". (b) **A resolved outcome is stamped ONCE and never recomputed** (the v129 frozen-marks rule) — that is what decouples the record from the data, so the producer only needs fresh prices for episodes still RUNNING, which makes `gradingUniverse` **self-draining** rather than unbounded. (c) **A resolved trade returns at its EXIT, not marked to today** — ORCL rendered "⛔ STOPPED · +18.3%" (it stopped at 127.94 on 07-14, then rallied to ~158 without us); averaging that in measures a position nobody held. Exports `closeSeries` · `pickEpisodes` · `gradeEpisode` · `gradeAll` · `gradeStats` · `gradingUniverse`, plus a CLI (`--symbols` feeds the fetch step, `--json` dumps the ledger). `GRADE_HORIZON_DAYS` (60) closes the window — a target hit two months after a 4-8 week swing scan is not that pick working — into `EXPIRED`; `BAR_REACH_DAYS` (5) tolerates a long weekend. `index.html` **mirrors it verbatim** as the fallback for pre-2026-09-09 snapshots (`PICK_GRADE_HORIZON_DAYS`/`PICK_BAR_REACH_DAYS`) — **keep the two in step**; a card grading differently from the producer is exactly the drift this repo already calls worse than no badge. |
...
| `producer/alerts.mjs` | **Pure level-crossing detection** (v143: also the **brokerage-basis RESIDUAL** — `total_value` carrying value that is not holdings, options, cash or a modelled derivatives sleeve, i.e. an asset bucket the producer has never heard of, which makes the recorded basis and therefore `cumFlow` wrong. Transition-based, so an unexplained bucket says so once rather than every run; it pushes because the guard it replaced only warned into a scheduled run's log. v121: also the **book-level drawdown TIER CHANGE** — `ok→soft→hard` and every recovery. Transition-based like the rest, so the owner is told once when the breaker trips rather than every 30 minutes while it holds. This one matters most: the breaker silently stops deploying and at the hard tier *sells*, so it has to reach the phone through the same push path as a stop crossing rather than only living on a card.) between the prior snapshot and the one being built (no I/O, unit-tested): agentic holding crosses its research **stop/target**, a top pick hits its published **TP1/TP2/stop**, a held name crosses **±7% on the day**. Transition-based (prior on one side, fresh through the other) so a crossing fires exactly once with no sent-state. `build-data` writes the result to `producer/raw/alerts.json`; the **agent** delivers post-publish via `PushNotification` (PRODUCER.md step 8, best-effort, every run); the Railway path only logs them. |
| `producer/markets.mjs` | `MARKET_SYMBOLS` (indexes/risk/sectors/intl) — source of truth; keep PRODUCER.md's list in sync. |
| `producer/analyze-universe.mjs` | **THE ANALYZE BENCH (v141)** — one ordered union of every symbol the dashboard can be asked to analyze, where **the order IS the fetch order**: holdings (both accounts) → agentic target → `MARKET_SYMBOLS` → `LEADER_SYMBOLS` → **SD_BENCH** → `RESEARCH_SYMBOLS` → VTI/GLDM. 181 static names (`--stats`/`--symbols` CLI); `analyzeUniverse({positions,target})` folds in the live half, `liveUniverse()` reads it off `raw/positions.json` + `raw/agentic-positions.json` + the committed `agentic-target.json` (best-effort — a missing input yields a SMALLER fetch list, never a wrong one). Consumed by `hist-plan.mjs` (which series to keep fresh) and by `av.mjs`'s `wideCover` (which names get fundamentals). **`SD_BENCH` is a COPY, and the test is what makes a copy safe** — `index.html` is one static file served to a phone and cannot import a repo module, the same problem `finalists.mjs` has with the research workflow, so `analyze-universe.test.mjs` regex-extracts the real `SD_BENCH` object out of `index.html` and asserts the mirror neither drops a ticker nor invents one. A drifted mirror would quietly pull names out of the fetch rotation, which is precisely the staleness failure the bench exists to end. Adding a name only ever costs bytes, so err toward inclusion — but note the ceiling: the bench is what the producer PROMISES to keep fresh, and `HIST_FULL_CAP` is what it can actually seed per day. |
| `producer/hist-plan.mjs` | **The historicals planner (v141, mirrors `av-plan.mjs`)** — decrypts the committed snapshot best-effort, reads how far each bench symbol's series already reaches, and PRINTS the batches the agent must fetch (`--json` for tooling). It calls no MCP tool and writes no raw file. **Three groups, and the split is the whole idea:** **TAIL** (last real bar ≤ `HIST_TAIL_DAYS` 10 days old) → ~7 bars, **`HIST_TAIL_BATCH` 8 symbols/call**, `hist-day-tail-<n>.json`; **FULL** (absent, or staler than a tail can bridge) → YTD, ≤3/call, capped at `HIST_FULL_CAP` **30/run** in priority order so the bench seeds over ~7 FETCH_ALL days (the deferred count is printed); **MONTH** (no 5Y series or last bar > 35d) → ≤3/call, cap 15/run. It also prints the **quotes bench** batches (`quotes-bench-<n>.json`, 60/call) so every analyzable name carries a live price — `build-data` already reads any `quotes*.json` and carries quotes forward per-symbol (v88), so light runs need nothing. **Why 8 is safe for a tail while FULL stays at 3:** the ≤3 rule is about PAYLOAD SIZE (an oversized result is spilled to a temp file the agent must not `cp`), and a 7-bar tail is ~1/20th of a YTD series, so eight symbols of tail is smaller than one three-symbol YTD call. **`HIST_TAIL_LOOKBACK` (12d) must exceed `HIST_TAIL_DAYS` (10d)** — the tail window has to reach back PAST the last bar we hold, or `mergeBars` faithfully preserves a hole in the middle of the series; anything older is FULL on purpose, because a gap is worse than a refetch. **Freshness reads the last NON-interpolated bar** — a placeholder is a calendar slot with no close, and counting it would let a padded series masquerade as current. **Fails OPEN toward fetching** (no snapshot / no `PF_PASSPHRASE` / decrypt failure ⇒ everything is FULL, capped), and prints **symbols only** — never a quantity or a dollar figure, since its output lands in a scheduled run's log. |
| `producer/momentum.mjs` | **The momentum sleeve, as code (2026-09-24, pure + unit-tested).** `momentumFor` / `momentumScores` / `closes` / `sma` / `ret`. It was one of four sleeve AGENTS, and the one fetching hardest — Robinhood historicals, 3 symbols a call, across the whole bench — for a score that is arithmetic over daily closes the producer ALREADY HOLDS (`data.hist.day`, kept fresh for ~230 names since v141). Same move the flow sleeve made and the same one `research-universe.mjs` recommends in its own header. Returns `{scores:[...]}` — the exact sleeve-agent contract — so it drops into the workflow as `args.momentum` with no downstream change; absent, the workflow falls back to the agent and SAYS so. **The rubric is `sdMomentum`'s deliberately** (6m/3m RS vs SPY weighted .30/.25, price vs 50/100-DMA with slope .25, proximity to the trailing high .20) — re-deriving a second momentum definition for the agentic book is how this repo produces drift. **50/100, not 50/200:** a YTD daily series cannot reach 200 bars, so a 200-DMA would be null for most of the bench most of the year; the agent prompt said "50/200-DMA" and was quietly scoring against whatever it could compute. **The staleness gate is the load-bearing part** — `data.hist.day` goes stale PER SYMBOL and a series that stops STOPS AT ITS OWN HIGH (the 2026-08-14 MU/WULF/NBIS phantom 10.00 at "0.0% off the high"): past `AGING_DAYS` (35) a name ABSTAINS rather than scoring, and 11-35 days scores but carries `AGING_CONF` (0.75) so a stale name can never outrank a fresh one at equal merit. `absent ≠ zero` throughout: no quote, too few bars or fewer than two scorable components all return `status:'missing'` with no evidence, never a neutral 5. |
...
| `producer/extfund.mjs` · `producer/extfund-fetch.mjs` | **Supplementary fundamentals** (Finnhub + Financial Modeling Prep). `extfund.mjs` = pure normalizers turning each provider's payload into the **same COMPANY_OVERVIEW shape AV uses** (AV's fraction conventions: RevGrowth/Margin/DivYield stored as fractions — Finnhub returns percents, so ÷100; FMP TTM ratios are already fractions; mktcap → whole dollars). `extfund-fetch.mjs` fetches over HTTP (once/day ET gate, like `av-fetch.mjs`), writing `producer/raw/ext-fund/overview-<SYM>.json`. **AV stays PRIMARY** — `build-data.mjs` reads ext-fund *after* av-src and only **fills fields AV is missing** for a name (so AV's ForwardPE/AnalystTargetPrice win) or adds a whole overview for names AV's daily cap skipped (rich, so it beats the RH synth). Both providers independently optional (`FINNHUB_KEY` / `FMP_KEY`); no key → silently skipped. Unit-tested offline (`extfund.test.mjs`). **v141 — the cover is the ANALYZE BENCH, and FMP ROTATES.** Cover was `coverFromRaw` (the top ~14 holdings by value) — the *Alpha Vantage* cover, borrowed because these providers were bolted on beside AV. But they are not on AV's 25/day budget, and the consequence was that every bench name the Analyze tab can be asked about had no overview at all, so its `_fundScore` abstained and the read was technicals-only. `wideCover(rawDir, bench)` (`av.mjs`) now hands `extfund-fetch` the whole bench, holdings-first. **Finnhub takes all ~181 names each day** (2 calls/symbol) through a **sliding-window limiter at 55/min** — paced SEQUENTIALLY, not `Promise.all`'d, or the window under-counts by exactly 2x — and a 429 or a `limit/quota` message sets `limitHit`, which STOPS the run cleanly and keeps whatever landed. **FMP cannot**: 5 calls/symbol against ~250/day is ~45 names, so `fmpRotation` takes the **least-recently-refreshed first** off `_extAsOf`, a `YYYY-MM-DD` stamp `build-data.mjs` writes onto each refreshed overview and **carries forward**. Three things are load-bearing: the clock must live in the SNAPSHOT, not a `raw/` marker (raw/ is wiped every scheduled run, so a marker rotation would reset daily and re-cover the same head of the list forever); a name with NO stamp sorts FIRST, because never-covered beats stale-covered; and the stamp is written whether or not a field was actually FILLED, since the question is "when did we last spend calls here", not "when did we last learn something". `_extAsOf` is an extra key on an object every consumer reads BY NAME (`ov.ForwardPE`, `ov.Sector`, …) and that `parseAV` returns verbatim, so nothing renders it — and the AV-rich clobber guard and the fill-only-missing rule are both unchanged. |
| `producer/optvol.mjs` | **The realized-vol IV proxy behind every ESTIMATE-path option premium (2026-09-12, pure + unit-tested).** `realizedVol` / `volFromBars` / `buildIvBySym`. It was an inline loop in `options-build.mjs` that read ONLY `raw/hist-day*.json` and skipped any batch under 20 bars — fine until **v141 moved historicals onto 7-bar TAILS**, after which nearly every batch fails that gate, `ivBySym` comes back EMPTY and every estimate falls through to `ivProxyFor`'s flat **0.55/0.60** default. Measured on the live 2026-09-11 snapshot: the **RCL cash-secured put** advertised **~$1,305 income at 58% annualized** off a $13.05 premium that implies **~68% IV**, against RCL's real **23.2% (30d) / 32.2% (60d)**; the honest figures are **$511 at 23%**. FER's CSP read 60% against a true 13%. Replayed over the live snapshot the proxy goes **0 → 221 symbols on a light run**. **Three things are load-bearing.** (a) **The window is HORIZON-MATCHED** — `VOL_WINDOW_BARS` (60) ≈ two option horizons; the old loop scored the whole YTD file, which for RCL is 49.0% and still ~2x the honest number, because what a name did in February does not price a 34-day contract. (b) **It reads through `pickgrade.closeSeries`, the SHARED bar reader** — the old loop read `b.close_price ?? b.close`, and `close` is NEITHER bar shape, so it silently produced nothing on a Railway-normalized series; a fourth private coalescer is exactly what this file warns against. (c) **A degenerate series ABSTAINS rather than scoring 0** — a flat series would price every contract at the $0.05 floor and make a worthless option look free, so out-of-band results return null and the caller falls back (`absent !== zero`). Sources are raw-first, prior-snapshot-as-fill, and a 7-bar tail correctly yields null and **falls through** instead of blocking the deep series — that fall-through IS the fix. **Known residual, deliberately not modelled:** realized vol sits a few points under implied (the variance risk premium), so short-premium ideas now read slightly conservative and long-premium ideas slightly cheap. The principled next step is preferring REAL IV where the book already has it (`data.options.ivObserved`, `liveBySym`) — not inventing an uplift constant nobody can validate. |
| `producer/av*.mjs`, `options*.mjs` | Alpha Vantage wiring; options analysis. **v119 fixed the money units and the collateral rules — see the Gotchas entry; `options.mjs` now exports `positionPremium`/`orderPremium`/`sharesLockedByShortCalls`, all unit-tested in `options.test.mjs`.** `options.mjs` builds the ideas — single-leg long-call/covered-call/CSP (live-priced, delta-targeted) **plus estimate-only defined-risk structures (call debit spread, collar)** with a `legs[]` array the consumer draws as a combined payoff; estimate premiums use a **per-symbol realized-vol IV proxy** (`optvol.mjs` — see its row; it was silently empty from v141 until 2026-09-12, which made every estimate-path premium a flat-60%-IV guess). `options-build.mjs` analyzes your contracts (full greeks incl. **vega/gamma**, concrete **roll suggestions**, a portfolio **exposure** roll-up, `ivObserved`) and, on FETCH_ALL, emits `producer/raw/option-watchlist.json` (the single-leg ideas that resolved to a live contract `optionId`) for the options-watchlist sync. The agent records each idea's resolved `optionId` into `option-quotes.json` (see `options-plan.mjs`) so `options.mjs` can carry it onto the live idea. |
...
| `producer/AGENTIC.md` | **Runbook** for the agentic account: tax/reg rules (taxable, short-term lots, wash-sale, instant settlement + the v98 PDT day-trade guard), the weekly research→target→rebalance-proposal flow, execution policy (**alert, then the owner's confirm** — `agentic-confirm.mjs` or any Claude session), and the empty **PRE-AUTH template** for pre-approving one above-cap ticket. The weekly job is **wired into the producer** (step 7), not a separate trigger. |
| `producer/agentic-due.mjs` | **FORTNIGHTLY** gate (like `preflight.mjs`) for the agentic research refresh — `AGENTIC_DUE` (exit 0) / `AGENTIC_NOT_DUE` (exit 20). **It is a CALENDAR-WEEK test, not an age test (2026-09-03) — because an age test cannot be made to work here, and it ate a week TWICE.** Pure `researchDue(asOf, today)` + `weekStart` (weeks start Monday, matching the cron), unit-tested in `agentic-due.test.mjs` — the module had **no tests at all**, which is how the same defect shipped twice. Due whenever the committed target was not refreshed during the current ET week; `REFRESH_DAYS` (6) survives only as an absolute-staleness backstop. **Why the age gate was unfixable:** the Routine fires `12 11 * * 1` (Mondays) while `asOf` is stamped whenever finalize-target writes, so a threshold N blocks the Monday fire for every target written 1..N−1 days earlier — a whole BAND of weekdays. `>=7` blocked everything but the previous Monday: on 2026-08-31 the Routine fired, ran 109s, printed NOT_DUE and stopped — logging SUCCEEDED, because stopping cleanly IS success — and the target aged to 8d, pushing MA and V past `ENTRY_ZONE_STALE_DAYS` so the planner waived their entry bands. Dropping to **6** was justified by "every committed target landed on a TUESDAY" — **that was false** (2026-08-05, 08-12 and the current 09-02 are WEDNESDAYS), so the 2026-09-07 fire would have seen a 5-day-old target and skipped the week all over again. **Picking a smaller N only moves which weekdays are fatal.** Encode the cadence instead, and err toward running: a day early costs one pass, a miss costs a whole week and is invisible (what surfaces downstream is the planner quietly waiving entry bands). Producer step 7 runs the deep research only when DUE. **2026-09-24 — the period is now a FORTNIGHT**, anchored to `FORTNIGHT_EPOCH` (2026-01-05, a Monday) via `fortnightStart`, so "which fortnight" is a pure function of the date and cannot drift with when the last target happened to land; `REFRESH_DAYS` rises to **13** so the backstop still sits just inside one period (a test asserts `< 14`, or a stalled period would be invisible). The reasoning is the churn governor's own clock — `MIN_HOLD_DAYS` and `REENTRY_COOLDOWN_DAYS` are both 14, so a weekly target spent half its life proposing trades that could not execute — plus the per-agent cost above. **The cron is unchanged**; the gate simply prints NOT_DUE on the off weeks. **The asymmetry got more expensive, so the bias matters more:** running a period early costs one pass, missing one now costs a FORTNIGHT of advisory entry zones rather than a week. Still err toward running. **v93:** the gate is now also tripped EARLY by `agentic-triggers.mjs` (a deposit or a big held-name gap → `refreshResearch`), so research doesn't wait out the week when something material happens. |
...
| `producer/agentic-deploy.mjs` | **Pure deployment/rebalance planner (v93, unit-tested; FULL-BOOK since v96; LIMITED MARGIN since v98).** `planDeployment({target, positions, cash, quotes, earnings, washMap, crossActivity, accountActivity})` → a ready-to-execute ticket, enforcing the execution discipline that was previously only prose: **earnings blackout** (never deploy NEW money into a name ≤7d from its report — wait for the print), **gap-through-entry re-verify** (a name at/below its target stop = broken → defer; below its planned entry zone = thesis in question → defer, the GOOGL-after-earnings case), **wash-sale** (reads `recentLosses`), and **cash-flow-first / sells-before-buys** sequencing. **v96 additions** (the planner used to see only target names — 40% of the book once sat invisible in research-dropped names): **off-target exits** (a held name absent from the target = explicit SELL-to-exit), **tax-aware sale ordering + estimates** (losses first, per-sell est. ST P&L, `taxSummary` netting), **opportunistic TLH** (target name underwater ≥ max($75, 5% of cost) → harvested whole, wash-blocked from the buy legs), **cross-account wash guard** (`crossActivity` = recent margin-book buys; a recent buy there kills a harvest / flags an exit `washRisk` — executor fetches this live), and the **auto tier** (`autoEligible` = turnover ≤ `AUTO_TURNOVER_CAP` **$10,000** — owner-approved unattended-execution ceiling; $500 → $1,000 on 2026-08-11, then **$1,000 → $10,000 on 2026-08-25** because routine rebalances kept landing just over the tier and stalling on a confirm the owner was approving unread — an approval step nobody exercises is not a control, it is latency. The consumer mirrors it as `AG_AUTO_CAP` in `index.html`; **keep the two in step**). Buys move each underweight name only toward its (already cluster/vol-capped) target, so honoring the target respects the risk caps. Mirrors the consumer's Agentic-card badges so card + ticket agree. **v98 — LIMITED MARGIN (2026-08-11):** ••••3900 was upgraded from a cash account, so proceeds are spendable the moment a sell **fills**, with **no borrowing or leverage** (`unleveraged_buying_power == buying_power` — "unlevered 1×" still holds). (a) **The T+1 leg is gone** — one allocation pass over `cash + proceeds` (also better sizing: leg-1 used to pro-rate over settled cash alone and top names up a day later); `buysT1` is always `[]`, kept in the shape only so tickets written under the old two-leg model reach `done`; `buysNeedProceeds` flags when the buys require the sells to fill first. (b) **PDT now applies** — a limited-margin account is a margin account for FINRA, so 4+ day trades in 5 business days under $25k restricts it, and this book is ~$5k with an hourly executor. The guard is categorical, not a counter: `accountActivity` (`{SYM:{lastBuyDate}}`) refuses to sell **anything bought today** (→ `blockedSells`), across exits/harvests/trims — zero round trips ⇒ PDT can't accrue. The gate can't see today's fills, so the **executor supplies `accountActivity` live** from `get_equity_orders` on ••••3900.  **v121 — REGIME-AWARE PACING.** The idle-cash deadline fired identically at VIX 12 and VIX 35. `marketRegime(vix)` (pure, from the new **`data.vix`** block build-data emits off the already-recorded `INDEX_DATA` VIX) returns `calm`/`elevated`/`stressed` at the **same 22/30 bands the consumer's `azVix` uses**, and stretches the deadline ×1.5 / ×2 and halves the forced tranche when stressed. It also adds a **`regime` deferral** for the one genuinely bad corner: a **stressed tape PLUS advisory (stale >7d) entry zones** — band discipline is switched off exactly when price is moving fastest, and buying unbanded into a stressed tape is the worst of both (a stale target in a CALM tape still buys, per the v102 lesson that stale zones must not defer the whole book). **Regime never picks names or sizes positions** — only pacing — and never blocks a sell. Missing/unparseable VIX ⇒ `calm` ⇒ today's behaviour exactly. **v121 — BOOK-LEVEL DRAWDOWN BREAKER.** `planDeployment({drawdown})` takes `drawdown.mjs`'s read (absent/`ok` ⇒ today's behaviour exactly). **soft** (≤−5pp BEHIND SPY since Mandate A, 2026-09-08 — was ≤−8% absolute; see the drawdown.mjs row): every new buy defers with reason **`drawdown`**, which outranks every name-level guard because it is a statement about the whole book; **deferred dollars stay in CASH, not parked** — the owner kept VTI as the park vehicle (SGOV declined 2026-08-24), and VTI is 100% equity beta, so routing "the market is falling" money into it is precisely backwards; and the **idle-cash deadline is paused** (its clock keeps running, but "10 days elapsed" loses to "the book is in a drawdown"). **hard** (≤−8pp relative, or the −20% absolute backstop): also raises defensive cash to `AG_DD_CASH_FLOOR` (20% of book) via `kind:'drawdown-raise'` sells, **losses-first** like every other sell path here. **The suspension flag is `parkNewOn`, NOT `parkingOn`** — that distinction is load-bearing: `parkingOn` also gates the off-target-exit exemption for the vehicle, so flipping it would make the planner liquidate the *existing* waiting ground as an "orphan" the moment a drawdown began, which is the infinite park→liquidate churn the exemption exists to prevent. Sells, exits and TLH are **never** blocked by the breaker (de-risking must always be possible), and the breaker does **not** override the PDT day-trade guard or the 14d min-hold — it warns when the cash floor can't be reached rather than forcing past them. **v102 — entry-band discipline + the waiting ground.** The zone check was a one-sided bright line (`px < entryLow`, no tolerance, nothing above) and failed three ways on 2026-08-11: V at 0.2% under a floor parked $437 (**no tolerance**); THREE of seven names read "below entry" at once purely because the zones were six days stale (**no ageing**); and the re-verification deliberately set every zone *below* spot, which the planner would have bought straight through because **"too expensive" had no code path**. Now: a SYMMETRIC band — `ENTRY_TOLERANCE_PCT` 2.5% under the floor, `ENTRY_PREMIUM_PCT` 2.0% over the ceiling (new `above-entry` deferral) — zones go **advisory** past `ENTRY_ZONE_STALE_DAYS` (7, off `target.asOf`), and `below-stop` stays absolute and unbanded because that one is the real thesis-broken signal. **Idle-cash deadline:** nothing ever forced cash IN, so "wait for a pullback" had no expiry and a rising market could strand a deposit indefinitely; past `CASH_IDLE_DEPLOY_DAYS` (**5** since Mandate A — cash is drag on a book paid to beat a fully-invested benchmark; was 10, from `data.agentic.cashIdleSince`) the bands are waived and cash deploys in `CASH_IDLE_TRANCHE_PCT` (34%) tranches. **DEPOSIT TRANCHING (Mandate A, 2026-09-08)** is the mirror of that at the other end of the same clock: fresh cash is rationed to `DEPOSIT_TRANCHE_PCT` (50%) per pass for `DEPOSIT_TRANCHE_DAYS` (3) above `DEPOSIT_TRANCHE_MIN` ($500), because every deposit to date was deployed IN FULL at market the day it landed (8/11 $5,000, 8/25 $1,400, 9/8 $1,815) into names the model already wanted more of — the maximum-variance way to add money, and exactly what made every deposit feel like it went straight into the red. It reuses `cashIdleDays`, so it needs **no new fetch, no new committed state and no Routine prompt change**; proceeds are never rationed (a rebalance is already in motion); and the idle deadline still forces the remainder in, so it can **delay a buy and never veto one**. **ONE TRANCHE PER DAY is the condition that makes it a tranche at all (verify catch, 2026-09-08):** the executor runs HOURLY and `idleDays` only advances at midnight, so the first build rationed the *remainder* by 50% again on every pass of deposit day — $2,000 went in as $1,000/$500/$250/$250 across four consecutive hours, fully deployed in ~3 hours with the variance reduction never happening. The build's own test ran a single pass and could not see it. `depositWait` now holds fresh cash back once ANY buy has been placed today (read through the PDT guard's own `boughtToday` predicate, so "today" can never mean two things), and the wait is decoupled from the split floor — `DEPOSIT_TRANCHE_MIN` decides whether a balance is worth SPLITTING, never whether it WAITS, or a $600 deposit went $300 + $300 in consecutive hours. Resulting schedule on a lump: ~50% day 0, ~25% day 1, ~12.5% day 2, the rest by day 3. The old idle-cash tranche is unchanged, sweeping whole below `CASH_IDLE_SWEEP_FLOOR` ($250) so thirds-of-a-shrinking-balance can't run forever. **INDEX PARKING (`PARK_VEHICLE` = VTI, owner's design):** a deferred name's dollars go to a broad-market placeholder instead of idling in cash, and are released to fund the name when it clears. **Since 2026-09-03 `PARK_DATED_REASONS` is `{wash-sale}` ALONE — everything else waits in CASH.** It was first narrowed to the four dated reasons (wash-sale · reentry · earnings · policy) that morning, then to wash-sale only the same day when the owner **switched idle cash to a high-yield sweep**: parking only ever paid for itself by earning beta cash could not, and cash now earns a real rate with no spread, no ST taxable round trip and no wash exposure of its own. A 30-day wash wait is the one window long enough to be worth a spread (the 08-12 NVDA park, blocked to 08-28, is the one that earned its keep); earnings/policy blackouts are ~7d and a re-entry cooldown clears on a date the account chose.** `PARK_MIN` gates on SIZE, but the cost that bites is DURATION: the vehicle's round trip pays a full spread (VTI quoted 376.69/377.50 on 09-02 — 0.21%) to rent a few days of beta. Measured on the real book, the 08-26 park of $1,028.66 released 08-27 (+$1.78) and 09-02 (−$1.00) — **+$0.78 gross on ~$1,000 of turnover, less than the spread it paid**, and VTI itself fell 0.22% over the window, so cash would have done better. The one park that earned its keep was 08-12: NVDA wash-blocked to 08-28, a known 16-day wait. Dated deferrals run multi-day to multi-week by construction; undated ones clear when the MARKET moves (1-6 days live) and can clear on the very next pass. **Cash is the safe failure mode** — it cannot lose to a spread, and the idle-cash deadline still backstops it, so this delays a buy at worst and never vetoes one. Replayed against the live 09-03 book it keeps **$916.29** (ELV no-quote + MA above-entry) out of a round trip. **VTI, not SPY** — SPY is the target's own ballast, so parking there buries the placeholder inside a position that has its own weight; a separate ticker makes it visible, and VTI/SPY track different indexes so the two never become substantially identical for wash-sale purposes. **Not QQQ** (~half megacap tech = the very cluster `riskweights.mjs` caps at 48%). Two load-bearing invariants: the vehicle is **exempt from off-target exits** (the v96 orphan rule would otherwise liquidate it every pass while parking rebuilt it — an infinite taxable churn loop), and **unparking is a taxable ST sale**, so releases are floored at `PARK_MIN` ($100), sized to the actual shortfall, and run through the same losses-first ordering and PDT guard as any other sell. Plan carries `entryPolicy` + `parking`. **2026-08-28 — FUNDING RECONCILIATION (the floor is inherited by the BUY).** The pool counts the waiting ground, but the release that frees it can decline to fire — shortfall under `PARK_MIN`, the parked block itself under it, or the day-trade guard bouncing the leg — and the buys were sized as if it had. Live: $485.99 parked against $0.90 cash, a $26.94 JNJ top-up needing $26.04 of release, the floor correctly suppressing the dust sale, and the buy shipping anyway. Buys are now re-sized against cash+proceeds alone when no release materializes, so a sub-floor top-up waits (0.23pp of drift is not worth a taxable ST sale of the placeholder — the same judgement `PARK_MIN` makes about the sale). Backed by a hard invariant: **spend ≤ cash + proceeds + an actual release leg**, breach ⇒ a `PLANNER BUG` warning instead of a quiet unfundable ticket. **2026-09-08 — that invariant caught its first real breach, in the re-size itself:** each leg is rounded to cents INDEPENDENTLY, so a pro-rated pass across ten names summed **$1,817.31 against $1,817.29** of cash — two cents over, enough for the broker to reject the last leg. `allocate` now caps every leg at what is actually LEFT (`room = pool − total`, rounded DOWN — the residue is a cent of undeployed cash the next pass collects, where rounding up is a rejected order) and breaks once the remainder falls under `MIN_BUY`. **Sizing against a pool is a promise not to exceed it, and the promise has to be enforced rather than left to the rounding to honour.** **CHURN GOVERNOR (2026-08-12).** On 08-10 the planner exited GE/LLY/AMZN/MSFT (dropped by the 08-05 target) and bought AAPL/UNH/V; on 08-12 the next target reversed BOTH legs — a near-total book flip in 48h, every leg ST-taxable, because nothing priced the cost of changing our mind (memoryless weekly research × full-delta execution). Three deterministic guards now: **min-hold** (`MIN_HOLD_DAYS` 14 — **PER LOT since 2026-09-09**: only the SHARES bought inside the window are held, and an oversized sell is RESIZED to the free older shares with just the remainder reported in `blockedSells` under its own unlock date; overridden only by a `target.dropped` **business-broken** verdict, a ≤`MIN_HOLD_EXEMPT_LOSS_PCT` (−10%) loss — risk control outranks churn control — or by being a TLH harvest/park-release; day 0 stays the harder PDT block. **It was per NAME until 2026-09-09, which is the wrong unit:** SPY took a $200.73 top-up on 09-08 (one leg of a deposit tranche) and that froze the whole ~$2,265 SPY position until 09-22 — and since the new target's SPY trim (17% → 4.5%, ~$1,675) was the *only* funding for its four new buys, the first Mandate A rebalance would have idled for a fortnight over a $200 lot. The churn this guard exists to stop is flipping a just-opened position, not trading shares held for months that happen to share a ticker with a recent top-up. Lot detail comes from `activityFromDecisions`, which now emits `buys:[{date,dollars,shares}]` per symbol. **Three fail-safes, each because the guard must never unlock what it cannot measure:** an activity entry with NO `buys` array — the executor's live overlay writes only `{lastBuyDate}`, and older callers pass only that — falls back to the per-NAME block unchanged; an in-window lot whose `shares` is null (no `priceAt` on the recorded leg) locks the WHOLE position rather than being read as zero shares; and a free slice worth less than `MIN_BUY` ($25) is a full block, the same judgement `MIN_BUY`/`PARK_MIN` already make about a dust sale not paying for its own taxable round trip. **PDT stays per NAME** — FINRA counts a day trade per SECURITY, so `dayTradeBlock` runs first and is untouched, and day 0 is therefore excluded from the lot lock), **re-entry cooldown** (`REENTRY_COOLDOWN_DAYS` 14 — a name this account sold inside the window is not rebought; deferral reason `reentry`, weight parks in VTI like any deferral; covers the GAIN-sells the 30d wash ledger can't), and a **dust floor** (`MIN_BUY` $25 — the 08-05 ticket placed a $1.80 UNH buy). A `phaseOut` target name (below) is held but never bought into and never parks. `accountActivity` gained `lastSellDate`; the exec gate now derives it from the committed decisions ledger (`activityFromDecisions`) and the executor overlays today's live fills. |
| `producer/agentic-pending.mjs` | **Pure rebalance-ticket state machine (v96, unit-tested).** The ticket lives in the COMMITTED `producer/agentic-pending.json` (raw/ is wiped every run — the target/ledger pattern): `proposed → confirmed → sells-placed → buys-placed → done` (or `aborted`), with `makeTicket` / `advanceTicket` (illegal transitions throw) / `nextAction(ticket, todayET)` (what the executor should do NOW: await-confirm · place-trades · place-buys once the sells fill · stale → re-plan after 5d) / `planHash` (a deterministic fingerprint of what the plan trades, so a materially-changed plan replaces a stale ticket rather than being mistaken for it). `build-data.mjs` attaches a live ticket as `data.agentic.pending` so the card shows "rebalance in flight" — **done/aborted tickets are omitted**, which is why the v126 blocked sells ride OUTSIDE it as **`data.agentic.blockedSells`** `{ticket,created,status,items[]}`: a FINISHED ticket is exactly when a held-back sell still needs explaining (the 08-25 ticket hit `done` the moment its buys filled while both exits were still blocked), so hitching them to `pending` would have shipped the fix inert. That block **self-expires by each item's `until`**, not by ticket age — once the unlock date passes the guard has released and the next plan emits the real sell, so a surviving row would claim a trade is held when it is merely queued. **v126 — the ticket also carries `blockedSells` + `warnings`.** It used to persist `deferred` only, which is BUY-side, so a ticket whose entire SELL leg was suppressed rendered as a pure buy list with the reason nowhere on the phone. That is exactly how 2026-08-25 read: a $1,400 deposit funded five buys while the JPM and GE exits the target called for sat **one day** short of the 14d min-hold (both bought 08-12, day 13), and the owner reasonably read deposit-then-buys-only as the sells having failed. The planner computed the blocks all along — the ticket dropped them, and `raw/agentic-plan.json` is wiped every run, so the ticket is the ONLY carrier that survives to the consumer. Each entry keeps `{sym, kind, blocked, dollars, shares, partial, pl, plPct, until, heldDays, note}` — `kind` is what the sell WOULD have been (exit/trim/harvest), `blocked` names the guard, `until` is the unlock date as a FIELD (the consumer must never re-parse it out of the prose), and `shares`/`partial` (2026-09-09) carry the per-lot min-hold split: an entry can now be the locked REMAINDER of a sell that partly went through, and how many shares are held is a field for the same reason the date is. A block is **not** a trade and deliberately does **not** enter `planHash`, or an unchanged plan would re-propose itself every pass as a day-count ticked. **The consumer cannot recompute this** — min-hold needs per-name `lastBuyDate`, which the snapshot doesn't carry. |
| `producer/agentic-confirm.mjs` | **The owner's confirm, as one command (2026-09-02).** `node producer/agentic-confirm.mjs <ticket-id\|latest> [--abort] [--note "…"] [--commit]` moves the committed `agentic-pending.json` from `proposed → confirmed` (or `→ aborted`) **through `advanceTicket`** — a second front door onto the same state machine, never a second machine, so an illegal transition throws here exactly as it would in the executor. It exists because the card, the push text and the runbook had promised a **"one-tap confirm" since v96 that did not exist**: the only way out of `proposed` was a human typing into a Claude session, which needs the connectors and is precisely the round trip the owner was complaining about. It places nothing and touches no broker — the next executor pass reads the confirmed ticket and places it. **`--commit` runs git through `spawnSync` with ARGUMENT ARRAYS, never a shell string**, because `--note` is owner text and the id comes off disk. Pushing matters: the executor runs from a fresh clone of `origin/main`, so an unpushed confirm is invisible to it — hence `--commit`, and the exact git commands printed when it is omitted. Refuses with a non-zero exit and a reason for anything not `proposed`, distinguishing already-confirmed (waiting on the executor) from done/aborted (history). Pure `confirmTicket` + a CLI, unit-tested against a temp file so the tests can never touch the real ticket. The other path is unchanged: tell any Claude session "confirm the pending rebalance". |
...
| `producer/finalize-target.mjs` | Turns the research workflow's raw `allocation` into the committed `agentic-target.json`, **deterministically re-enforcing** `riskweights.mjs` caps (the workflow proposes, this disposes — cluster/vol caps hold regardless of the LLM output). Producer step 7.4 pipes the workflow output through it; also `import { finalizeTarget }` for on-demand rebalances. **Two-strike phase-out (2026-08-12 churn governor):** a HELD name in the prior target but missing from the new allocation is **retained at its prior weight, flagged `phaseOut:true`** (zero trades — if next week re-includes it, nothing was ever sold); still missing on the NEXT refresh → genuinely dropped; an explicit adverse verdict (`businessOk:false`/rec `avoid`) drops immediately. Drop reasons land in **`target.dropped`** (`business-broken` unlocks the planner's min-hold). The CLI auto-reads the committed target as `prior` (`--no-prior` to disable) and takes `--held SYM,…` (••••3900's current tickers — without it every prior name is protected, which fails safe since the planner never BUYS a phaseOut name); `verdicts` come from the workflow return. **v124** also applies the **defensive floor** and emits `target.defensive` `{direct,lookThrough,total,floor,shortfall}` (`meta.defensiveMin` overrides; `0` disables). **Also derives the ENTRY BAND from the adversarial verdict (2026-08-25).** `entryDiscountFor(entryQuality, bar)` → the discount below spot the zone's ceiling must sit at (1.5%/pt below the bar, capped at `AG_ENTRY_Q_MAX` 8%). **THE BAR IS THE COHORT MEDIAN SINCE MANDATE A (2026-09-08), NOT A FIXED 6** — `cohortEntryBar(verdicts)`. The absolute bar was a backdoor MARKET-TIMING lever, which is the one thing this mandate forbids: entryQuality verdicts cluster hard (2026-08-25 returned all sixteen between 2 and 5, **ten on exactly 3**; 2026-09-07 had nine of fifteen ≤4), so against a fixed 6 *almost every name* was haircut at once — and since the synthesis normalizes to ~100%, a uniform haircut cannot change the ordering and the freed weight had nowhere to go but the index core and the ballast. The 2026-09-07 target says it in its own summary: *"Risk posture is defensive-tilted for a book where nine of fifteen verdicts scored entryQuality ≤4."* Measured against the median instead, a uniformly-extended cohort yields **zero** haircuts — the same book at the same weights, which is right, because "all sixteen are a 3" is information about the tape and none about which of the sixteen to buy. One absolute component survives: `AG_ENTRY_Q_FLOOR` (2) still tightens a genuinely chased entry even when its peers are equally bad. **`absent ≠ zero` bites TWICE here** — `+null` is 0 AND `Number.isFinite(0)` is true, so a naive finite-check on the bar would set it to 0, make every name "better than the cohort" and silently disable the guard rather than falling back. The workflow mirrors the same rule for the WEIGHT haircut (centred on the median, bounded 0.70-1.20); keep the two in step. **The defensive exemption is now conditional on the floor being active** — with `AG_DEFENSIVE_MIN` at 0 there is no mandated ballast to protect, so a defensive name is a pick like any other and faces the same entry discipline; `tightenEntryByQuality` rewrites the zone so the deploy planner's `above-entry` deferral actually fires. Closes a real hole: `entryQuality` was documented as "sizes, does not veto" and did shrink the WEIGHT, but never moved the ZONE — and the v102 prompt tells the model to set *reachable* zones, so it brackets spot and a 3/10 entry executed at market exactly like a 9/10 one. Four guards: the discount is **bounded** (a deep zone reads as never-buy — the v102 lesson), the **idle-cash deadline still backstops it** so it can delay a buy but never veto one, it **never loosens** a zone the model set tighter, and **defensive-floor names are EXEMPT** (deferred weight parks in VTI, 100% equity beta, so deferring ballast would swap stabilizers for beta at the worst moment — same reasoning as the drawdown breaker holding deferred dollars in cash). **`absent ≠ zero`:** `+null` coerces to 0, so an unscored name would have drawn the maximum haircut; a missing verdict leaves the zone untouched. **The "gold diversifier added structurally" note in `method` keys on an ACTUAL injection (2026-09-09), not on gold being present** — with the floor at 0 a GLDM row still reaches the target as a phase-out retention, and the first Mandate A target claimed a sleeve had been added when nothing was; the numbers were right, the prose was false (pinned by a test). **v126 — `dropped` SURVIVES A RE-RUN.** Drops are detected only by diffing the new allocation against `prior.names`, and the CLI auto-reads the COMMITTED target as `prior` — so re-running finalize against its own output finds nothing missing and writes `dropped: []`, **erasing the record**. Live on 2026-08-25: three finalize runs landed the same day (research refresh → entry bands → gold sleeve) and the JPM/GE `phase-out-complete` entries the first wrote were wiped by the second and third. Benign for a phase-out; **not** benign for `business-broken`, the one reason that unlocks the deploy planner's 14d min-hold — losing it would strand a broken thesis in the book for two more weeks. A prior record is now carried forward while it is still TRUE: still absent from the target, still held (when `held` is supplied), and inside `DROP_RETENTION_DAYS` (90). Each entry carries `since` (first recorded, never re-stamped) and carried ones are tagged `carried:true`; a freshly-detected drop always wins on conflict. **The natural terminator is the exit itself** — a name that leaves the book leaves this list — so the 90d cap is only a backstop for callers that omit `held`. |
...
- **Versioning:** any change to `index.html`/`sw.js` → bump **both** `APP_VERSION` (in `index.html`
  ...
  **DAY IN CASH** pass (2026-09-12, owner-set): the Agentic Portfolio table's `Day / YTD` column drops YTD
  ...
  column across** — two adjacent money columns that order their own two lines differently make the eye
  re-read the row to find the figure. Greying the sub-line percentage (`pctSub`, which keeps `pctCell`'s
  display-zero-aware formatting so a flat name reads `0.0%`, never `−0.0%`) is the substantive half of that
  match, not a cosmetic one: the dollars directly above always carry the same sign (both derive from
  `px − prevClose`), so colouring both states the direction twice and leaves nothing quieter to skip —
  exactly why `plCell` greys its own. The reason it is the better second line is that **a percentage
  cannot be added up** — "+0.6% and +0.8%" answers nothing about the account — while the dollars sum
  straight down the column into the Total row, which is exactly the question a holdings table is scanned
  for. The sum is over the same priced rows the percentage is computed over, so an unquotable name (the
  parking vehicle, typically) reads `—` on both lines and is excluded from both rather than silently
  counted as zero. `cashCell` carries the **display-zero rule** `pctCell` already had — a move that rounds
  to $0.00 renders neutral, never a red `−$0.00` — and the `+`/`−` convention matches `plCell` so the two
  money columns read alike. Per-name YTD is **not deleted, it moves to the cell's `title`**, since nothing
  else on the agentic side carries one (the Analyze tab is the other route). Privacy: the new figure is
  money and masks through the existing `$` pattern — re-audited, 0 leaked across all seven surfaces. v145
  is the **CONSTRUCTION NOTE** pass (2026-09-12): `target.method` is the only place the target explains ITSELF —
  why each name got its weight, and why five names left — and it rendered as one **2,892-character
  paragraph** at 11.5px with `|` swapped for `·`, which is a thing nobody reads. It is now chunked and
  weighted by `agMethodHtml` (with the pure `agMethodParts`/`agProseLines`/`agEmph`, pinned by
  `producer/agentic-method.test.mjs`, which extracts them out of `index.html` the way `analyze-core` and
  `finalists` do). **The rule is CHUNK AND EMPHASIZE, NEVER REWRITE, and the test asserts it** — every
  character of `method` must survive into the rendered output, in order (whitespace and the delimiters
  themselves excepted) — because a card that paraphrases the research is a card asserting something the
  research did not say, and a summary of a summary is how a figure that was already caveated becomes one
  that reads as current. Four things earn their place: the string is **two different artifacts glued with
  `|`** (the synthesis's prose, then the tail `finalize-target.mjs` appends), so it splits there and the
  tail segments are classified **by their emitted prefix, never by position** — an unrecognized future
  segment renders verbatim under a neutral `·`, because "shown plainly" is the only acceptable degradation
  and a formatter keyed on a fixed segment count silently eats the next one somebody adds; the prose breaks
  at semicolons **as well as** full stops, since the synthesis writes its whole drop list as one
  semicolon-joined sentence and that clause is exactly the unit the reader wants one per line; emphasis is
  **two weights, not one** (ticker `<b>` 700, figure 600) because the note carries ~30 percentages against
  ~20 tickers and bolding both equally lit up half the paragraph — and the ticker vocabulary comes from
  **the target's own `names`/`dropped`**, never a `\b[A-Z]{2,5}\b` guess, which would light up RSI, AFTER
  and COHORT; and the block now **leads with an authoritative header strip** built from the structured
  fields, which is also the first render `target.defensive`/`target.diversifier` have ever had (emitted and
  shown nowhere since 2026-08-25). That header is not decoration — the note's own last segment warns that
  percentages quoted in the prose can predate the final cap/sleeve pass, so the figures scanned FIRST have
  to be the ones that can't be stale, and it answers the question the prose opens with a contradiction to
  ("Twelve names" over a table of 18 → `12 active + 6 phase-out`). Escaping also landed here: the old site
  interpolated `method` into `innerHTML` raw. v144 is the
  **HOLDINGS ORDER** pass (2026-09-12, owner-set): every holdings listing in the app now reads
  largest-position-first. Most already did (All Positions defaults to `val` descending; the heatmap,
  Technical Signals and Fundamentals all sort `b.val-a.val`), so what stood out was the **Agentic
  Portfolio** table, which iterated `data.agentic.positions` in the producer's — i.e. the broker's —
  order and rendered SPY $593 · LLY $761 · NVDA $1,068 · V $871 · GOOGL $914 down a card whose whole
  job is to show where the money is. The sort lives in **`agenticBook()`**, not at the render site,
  because three surfaces read that one array (the card's holdings table, its off-target orphan list,
  and the Plan tab's off-target line) and sorting per-site is how this repo keeps producing drift;
  `agenticEnriched()` got the same ordering for the shared analytics renderers. Two details are
  load-bearing: the sort key falls back to `qty × px` when `value` is 0, because the card's own row
  builder does (an unquotable name prices at 0 in the producer, and an order that contradicts the
  figures printed beside it is worse than the broker's order); and the **deliberate non-value
  orderings were left alone** — the hand-off's orphan exits stay losses-first (tax), the margin
  paydown table stays weakest-momentum-first, the Flow card stays flow-score-ranked, ex-dividends
  stay by date. The 🧹 Positions Under $250 chips flipped from ascending to descending to match.
  v143 is the
  **RECORDED-BASIS** pass (2026-09-10): the self-directed account's equity series moves off `total_value` onto the
  brokerage book (`brokerageEquity`), after `total_value` was measured to INCLUDE the event-contract sleeve the docs
  had always claimed sat outside it; the consumer's only change is that the 💰 contributions strip skips the one-off
  `basisShift` point, so a change of measuring perimeter is not listed as a transfer the owner made (see the
  prediction-market gotcha and the `equityseries.mjs` row); v142 is the **PER-LOT MIN-HOLD** pass (2026-09-09): the 14-day churn guard stops locking a whole POSITION because part of it was bought recently — only the in-window lots are held, an oversized trim/exit is resized to the free shares and the locked remainder is reported with its own unlock date (see the `agentic-deploy.mjs` row); v141 is the **ANALYZE ACCURACY + WIDE FRESH BENCH** pass (2026-09-09, Plan→Build→Verify on three Sonnet research reports): every Analyze reader now goes through one staleness-gated, live-spliced `azHist` (technicals **withheld** past 10 stale days, haircut and badged at 4–10), the call is a PURE `azCore` with a real five-state label machine (the old ladder was effectively binary and its "Oversold Bounce" branch never fired), ATR-based stops decoupled from structure-based targets with reward:risk shown and enforced, a **📏 Measured in this snapshot** strip that walks every label forward over the whole universe's recorded closes, a rewritten Recommendation card (action · why · horizon · R:R · what flips it · data quality), the "Rev growth: NaN%" and trailing-P/E-as-forward leaks fixed, and on the producer side `analyze-universe.mjs` + `hist-plan.mjs` + `histbars.mergeBars` keep a ~230-name bench fresh for ~16KB/day (see the gotcha below); v140 **fixed the Daily-Picks Track Record**, which was reporting a **0% hit rate** it had no evidence for: episodes are now graded (and FROZEN) in the producer via `pickgrade.mjs`, a price series that does not reach the measurement date reports **can't be graded** instead of "open", a resolved pick is measured **to its exit** rather than marked to today, and every price on the card carries its as-of date (see the gotcha and the `pickgrade.mjs` row); v138 is the **Mandate A VERIFY** pass (the consumer half of the three seams the adversarial verify caught: the stale-zone asymmetry mirrored into `agenticExecMap`, and the drawdown banner/hand-off quoting the relative figure); v137 is the **MANDATE A** pass (owner-set 2026-09-08: the agentic account's job is to BEAT SPY over rolling 12-month windows, not to preserve capital): the defensive floor and gold sleeve go to 0, the index core becomes a 5-10% residual, the drawdown breaker becomes **relative to SPY**, deposits **tranche in**, the `entryQuality` haircut becomes **relative to the cohort median**, the stale-zone waiver becomes **asymmetric**, and both Rebalance Logs gain a **buys-only alpha** strip (see the mandate entry below and `AGENTIC.md`); v136 **reworked privacy mode** (👁️/🙈) from a one-page dollar blur into an app-wide, region-based redaction, and fixed a `dLbl` ReferenceError that blanked the entire agentic Plan tab whenever the drawdown banner rendered (both below); v135 carried the two owner decisions of 2026-09-03 onto the phone (**parking narrowed to wash-sale only** after idle cash moved to a high-yield sweep, and the **$25 de-minimis wash floor**); v134 gated every agentic **buy-side deferral on there actually being a buy** (the phantom "above entry" rows — see the gotcha); v133 replaced the **"one-tap confirm" wording** the app had never been able to deliver with the two real confirm paths (`agentic-confirm.mjs`, or a reply in Claude); v132 added the Income & Tax **prediction-market line item**; before that, — v127 added the self-directed Rebalance Log, v128 moved it to the Accounts tab beside the agentic one, v129 made both logs **accumulate a usable record** (frozen outcome marks + time-based retention), v130 **backfilled those marks from recorded daily closes** so the record starts full instead of empty, and v131 was the **navigation & accessibility pass** — bottom tab bar, section rail, Find, My view, ▲/▼ glyphs (all below).
- **••••3900 RUNS MANDATE A — BEAT SPY (owner-set 2026-09-08).** Objective: outperform SPY over rolling
  12-month windows. Unlevered, 10-12 names, research-driven, churn-governed, and it **accepts tracking
  error and drawdowns as deep as or deeper than SPY's**. **No structural ballast** — index core ≤10% (SPY
  as residual only), `AG_DEFENSIVE_MIN` **0**, `AG_DIVERSIFIER_MIN` **0**. Risk control is **relative**
  (the drawdown breaker acts on the book falling BEHIND SPY, never on it falling WITH SPY) and
  **correlation-based** (the cluster caps, which were NOT loosened and are now the primary downside
  control). **Never market-timing.** The trigger: the book was +4.40% vs SPY +3.94% since inception but
  −0.98% vs +0.56% over the preceding fortnight, and a bottom-up attribution put that gap in three names
  plus **structural ballast** (LLY −0.64pp, the floor+gold −0.54pp, BKNG −0.43pp) with execution costing
  ~zero — 42.5% of the book had zero or negative expected alpha in a rising tape. **Everything here is
  still MEASURED** (`target.defensive`, `diversifier`, look-through) — only the obligation went away, and
  each floor is a one-constant restore. Full rationale and the "what this will feel like" note:
  `AGENTIC.md` § THE MANDATE.
...
- **Track Record fixture (v140):** the sample snapshot now carries `picks.history` + `picks.grades`
  built by running the REAL `gradeAll`, with bars dated relative to TODAY (the rest of the fixture's
  bars are a fixed Jan–May stub, and a track record must reach the present or every episode is
  unmeasurable by construction). Five tickers cover all five outcomes — WINNER (TP2) · BANKER (TP1) ·
  BROKEN (stopped) · RUNNER (open) · **GHOST** (prices stop 3 days after the scan → can't be graded,
  listed twice so the episode dedupe shows). The card had NO fixture before, which is how it reported
  0% for weeks without anyone seeing it in preview.
...
- **Unit + integration tests:** `for t in producer/*.test.mjs; do node "$t"; done` — covers the
  ...
  **trend gate** and the **recent-stop-out cooldown** — `gradePickClose`/`recentStopCooldown`),
  `shapeSocial`, `computeAlerts`, **`agentic-exec-gate`** (the 2026-09-08 idle classification: each of the six unrecoverable reasons marked degraded, each self-clearing one left silent, the `[DEGRADED]` marker string the Routine greps for, and a COUNT of plain `idle()` calls so a new early-exit cannot be added without classifying it), **`pickgrade`** (the track record: both bar shapes coalesced and interpolated/live rows dropped, the episode dedupe and its gap boundary, **a series that stops days after the scan reported UNMEASURED rather than OPEN and carrying NO return** — the defect itself — a zero-bar episode likewise, the long-weekend reach tolerance binding on one side and not the other, **a STOPPED episode returning at its stop rather than at the rally that followed it**, TP1 banking so a later stop cannot undo the win, the stop tested before the targets on a shared bar, an intraday spliced bar unable to trip a stop, the horizon closing the window on a late target, **a frozen outcome surviving its bars going stale AND not being rewritten by richer later data** while an UNMEASURED one is still retried, a hit rate of `null` rather than 0% when nothing could be graded, and the fetch universe excluding resolved/expired episodes so it DRAINS), **`agentic-method`** (the v145 construction-note formatter, extracted out of `index.html`: the losslessness invariant itself — every character of `method` reaching the rendered output in order, against the LIVE committed target so the test tracks what actually ships — an unrecognized tail segment surviving verbatim under a neutral marker rather than being dropped, a semicolon-joined drop list becoming one line per drop, a decimal not read as a sentence end, the two-weight emphasis with RSI/AFTER/COHORT proved NOT to be mistaken for tickers, a single-letter ticker not matching inside a word, `ppm` not read as percentage points, markup in the note escaped rather than executed, emphasis never matching inside its own markup, and a target with no names/defensive/diversifier still rendering its note), **`histbars`** (the v139 bar compaction — the real live bar shape pinned verbatim, idempotency, series length preserved through a junk bar, a no-close bar passed through untouched, `h`/`l`/`v` dropped only where the reader's fallback matches, `interpolated:true` surviving while `:false` is dropped, and — the assertions that actually matter — `drawdown.benchSeries`, `maindecisions.spyClosesFrom` and a verbatim copy of the consumer's `azSeries` all returning BYTE-IDENTICAL output from the raw and compacted shapes), the **agentic v93 engines** (`riskweights` cluster/vol caps + the **v124 singleton-loophole closure** and **defensive floor** — the vol gate that disqualifies a wide-range pharma name, the per-cluster pooling that stops a top-up breaching the receiving cap, the never-fabricate shortfall report, and the `defensiveMin:0` off switch — **all now asserted with an explicit `defensiveMin`/`diversifierMin` since Mandate A set both defaults to 0 (migrated, never deleted: the mechanism must keep working because restoring either floor is one constant), plus the new default-path cases proving a megacap-only book is passed through untouched while the exposure is STILL measured**,
  ...
  gate, the megacap-tech exclusion), **`finalists`** (the challenger quota: reserved slots filled with non-incumbents only, the shared per-sector budget, merit backfill when challengers run out, the old top-10 behaviour reproduced at `challengerSlots:0`, the sector-balanced `universeSlice`, **and the mirror check that runs the workflow's own inlined copy against the module**), **`riskweights`** (the **gold diversifier**: floor top-up, the own-ceiling bound so it is not an overflow sink, never-a-donor to the defensive floor, never-fabricated when absent, and `diversifierMin:0` as a real off switch; plus the **look-through report-only migration** — caps binding on direct while the total stays computed and disclosed, and no false RESIDUAL on an unenforced basis), **`finalize-target`** (the **structural gold injection** — injected only when absent, at the normalization-surviving weight, tagged `Diversifier`, never overriding a vehicle the allocation already chose; the **2026-08-25 verdict-derived entry bands** — the discount curve, the 8% cap that keeps a zone reachable, never-loosens, the defensive-floor exemption, `absent ≠ zero` so an unscored name is not handed the maximum haircut, and the rewritten zone still parsing first-two-numbers as lo,hi per `agentic-deploy`'s contract; plus the **vol-gate regression** that a wide-range pharma name is excluded from the floor once px/hi/lo are supplied),
  the **Mandate A drawdown breaker** (`drawdown` — a market-wide fall reading ~0pp relative and NOT tripping where the v121
  absolute rule would have, an idiosyncratic fall of the same depth tripping soft, a −3% book against a +3% market tripping
  on −6pp relative where the absolute rule would have missed it entirely, the −20% absolute backstop binding on a crash the
  market shared, a STALE bench refused and falling back to absolute-only rather than blaming the book for a market-wide drop
  while the same decline against a FRESH bench still trips, no bench at all inventing no relative figure, hysteresis and a new
  peak re-anchoring the BENCHMARK as well as the book, and both bar shapes with interpolated/live/zero rows dropped), the
  **deposit tranche** (a lump rationed on day 0 and 1, whole from day 3, a sub-floor balance never split, proceeds never
  rationed, and the idle deadline still winning so it can delay a buy and never veto one), the **asymmetric stale-zone rule**
  (a stale target still waiving BELOW-entry per the v102 whole-book-stall lesson while ABOVE-entry survives staleness, with
  the live 2026-09-02 V fill at $378.30 against its own $372.50 ceiling pinned as a regression), the **raised TLH floor** (a
  loss that cleared the old $75/5% floor no longer harvested and so no wash block manufactured against a name the target still
  wants, plus the percent arm binding independently on a large shallow loss), **`buyStats`** (buy legs only, dollar-weighted
  so a dust winner cannot outvote a large loser, a well-timed trim excluded, abstaining rather than distorting when spyRet or
  a price is missing, and `stats`/`sleeves` left untouched), the **relative entry bar** (`cohortEntryBar` as the median, a
  uniform-3 cohort haircutting NOBODY where the absolute bar would have haircut every name, a name genuinely worse than its
  peers still tightened, the absolute `AG_ENTRY_Q_FLOOR` backstop still binding, an absent cohort falling back to the legacy
  bar rather than to zero, an end-to-end finalize run over a uniform cohort rewriting no zone at all, and the defensive
  exemption going inert once the floor is 0), the deterministic `drivers` attribution + the two-strike phase-out: strike-1 retention, strike-2 drop, business-broken and not-held drops, **plus the v126 same-day re-run carry-forward** — a record survives a finalize run against finalize's own output, keeps its ORIGINAL `since`, is not duplicated, is retired the moment the name is re-included or the position exits, loses to a freshly-detected drop, and expires past the retention window when `held` was never supplied — plus the **v124 defensive floor** reaching the committed target — shortfall reported on a megacap-only book, satisfied and silent on a book with real ballast, `defensiveMin:0` restoring pre-v124 behaviour), the **option money units + collateral rules** (`options.mjs` — per-contract vs
  per-order premium, the signed short credit, `sharesLockedByShortCalls`, and the covered-call idea
  guard), the **realized-vol IV proxy** (`optvol` — the horizon-matched window proving a calm recent
  regime is NOT blended with an old wild one, both bar shapes agreeing byte-for-byte, interpolated/live
  rows dropped, a short series and a FLAT series both ABSTAINING rather than scoring ~0 vol, junk
  out-of-band results abstaining, **a 7-bar tail yielding null and FALLING THROUGH to the deeper
  snapshot series — the defect itself**, an earlier source that does produce a figure still winning,
  tail+full concatenating and deduping by date, and an END-TO-END pass through the real `buildIdeas`
  pinning the live RCL case: the flat default reproduces the shipped $13.05 while a real 32.2% vol
  cuts premium, income, breakeven AND the headline annualized yield with it), the **recorded brokerage basis** (`equityseries.mjs` v143 — `brokerageEquity` over the live sleeve-INSIDE
  payload pinned verbatim, the same equity derived from a sleeve-OUTSIDE payload so the basis is
  convention-independent, the zero-sleeve account that reconciles vacuously and is what hid the bug, an unknown asset
  bucket surfacing as a residual rather than skewing the basis, the v116 loan term still binding, and the fail-back to
  `total_value` when components are missing; plus the one-time perimeter shift — booked as a flow so it cannot read as
  a loss, exempt from the noise floor because it is read not inferred, fired exactly ONCE, **surviving all ~13
  same-day re-runs with its flag intact** (the hourly-cadence case the deposit tranche taught), inert on a zero-sleeve
  account, and absent entirely when `sleeveValue` is not supplied), the **recorded equity series** (`equityseries.mjs` — the deposit inference incl. its
  ...
  trades are kept, and the retention window), the **weekly research gate** (`agentic-due` — every weekday a
  target could land on is due on the next Monday fire, a same-week refresh is not, the staleness backstop,
  fail-open on a missing/malformed target, and a future-dated target not forcing a refresh off clock skew),
  and a full `build-data.mjs` fixture run
...
- **`data.hist.day` goes STALE PER SYMBOL, and a momentum scorer reads stale bars as a perfect score.**
  ...
  **2026-06-18…22** (~8 weeks stale) and META 2026-06-26. This is lethal for any *ranking* built on those
  ...
  check the last bar's date against `_snapETDate()`, never just `bars.length`. **The producer-side fix
  landed in v141** — `hist-plan.mjs` keeps the whole Analyze bench fresh with 7-bar tails merged by
  `histbars.mergeBars` — but the gate stays, because the bench seeds over ~7 FETCH_ALL days, a name can
  still fall outside it, and a guard that costs nothing when the data is fresh is worth keeping. The Analyze
  page got the same gate in v141 (`azHist`); see the next entry.
- **A TRACK RECORD GRADED OFF A SERIES THAT STOPS MEASURES THE DATA, NOT THE STRATEGY (2026-09-09).**
  The Daily-Picks Track Record card read **0% hit rate · 0W·2L · 42 open · +2.1% avg return** — a
  shape that is internally impossible, since a positive average return alongside a 0% hit rate and
  42 "open" positions means almost nothing was ever graded. It wasn't. `gradePick` walked
  `data.hist.day[sym]` from each scan date, and that series **goes stale per symbol** (the gotcha
  above): the producer only bar-fetches its CURRENT rotation, so a pick's closes freeze within days
  of the scan that named it. **12 of the 44 episodes had ZERO bars after their own pick date** and
  most of the rest had fewer than ten. Regraded against the real closes: **9W·3L, a 75% hit rate.**
  **Four things generalize, and the first is the one worth carrying furthest.**
  **(a) Censoring is rarely symmetric, so "we can only measure some of it" is not a wash.** The
  window we retained was the few days right after each scan — and a stop sits ~6% away while a
  target sits ~13-24% away, so an early dip through the stop lands INSIDE that window while a target
  hit weeks later never does. The measurable slice could only produce losses and "open". A missing
  measurement is not neutral; work out which direction the gap points before trusting what's left.
  **(b) The two halves of the card were on different clocks, and that is what made it look wrong
  rather than merely empty.** Status came from bars that stopped in July; the return came from
  `data.quotes`, which **carries forward per-symbol** (v88) — so **35 of 42** archived names carried
  a frozen quote (VZ and T were two months old) rendered with no as-of date, as if live. The tell was
  a row reading "⛔ STOPPED · +18.3%": ORCL stopped at 127.94 on 07-14 and then rallied to ~158
  without us. **A resolved trade must be measured to its EXIT** — marking it to today prices a
  position nobody holds — and any figure that is not current must carry the date it was current.
  **(c) The fix has to move the measurement to where the data lives, not add more fetching.** Grading
  now happens in `build-data.mjs` via `pickgrade.mjs` and a resolved outcome is **frozen into the
  snapshot** (the v129 marks rule), so the record stops depending on the bars staying fresh forever.
  That is what makes the extra fetch **bounded and self-draining**: only episodes still RUNNING and
  inside their 60-day horizon are ever asked for (`gradingUniverse`, capped at 30), and each one
  leaves the list permanently the moment it resolves. A naive "just fetch all the pick history"
  would have grown without limit.
  **(d) It needed NO Routine prompt change, and checking that was the difference between a real fix
  and an inert one.** The producer Routine is the one `http_api` Routine here — its prompt is the
  owner's to paste, and cannot be edited from a session — so a fix requiring a prompt edit would
  have shipped broken. It doesn't: that prompt says "follow `producer/PRODUCER.md` exactly" and
  never enumerates the symbol lists, so widening the table's quotes/historicals rows is picked up on
  the next run. **Check whether the prompt names the thing you are changing before assuming either
  way** — CLAUDE.md's own rule is that a step naming a specific file or command overrides the
  runbook, and here none does.
  Preview it with the `make-sample-data.mjs` track fixture (WINNER/BANKER/BROKEN/RUNNER/**GHOST**) —
  the card had no local fixture at all, which is precisely how a card reporting 0% for weeks was
  never looked at in preview.
...
- **A PREDICTION-MARKET WIN LOOKS EXACTLY LIKE A DEPOSIT (2026-08-30).** The self-directed book settled a
  ...
  **The root cause is that the MEASURED book is the brokerage account only** — `equity_value + options_value +
  cash` — while `event_contracts_value`, `futures_value` and `crypto_value` are sleeves held by different legal
  entities (Robinhood Derivatives, LLC is a registered FCM; Robinhood Crypto, LLC). So money moving between the
  brokerage account and a derivatives sleeve is an **internal transfer the inference cannot see**. Fixed by
  ...
  **THAT DAY HAD ALREADY COME — AND THE "GUARD" WAS TESTING A PREMISE NOBODY HAD EVER MEASURED (2026-09-10).**
  This entry used to assert the sleeves sit outside `total_value`, "verified to 9 decimals on the live payload",
  with a guard for the day Robinhood folded one in. Both halves were wrong. `total_value` was measured against a
  live OPEN event position and it **includes the sleeve**: `41351.32550084 − 51 − 21146.93 = 20153.39550084`
  against a `total_value` of `20374.14550084` — a residual of exactly the `220.75` in `event_contracts_value`.
  Robinhood changed nothing; the premise was false from the start. **The identity is VACUOUS whenever the sleeve
  is empty**, and it was checked moments after the 08-30 settlement closed the position — the agentic account,
  same run, sleeve 0, still reconciles to nine decimals, which is precisely what made it look verified.
  **The generalizable lesson: an invariant that only holds trivially in the state you sampled is not evidence.**
  Before trusting one, ask which state would make it FAIL and check that you have actually observed that state.
  **What the bug did NOT do is the other half of the lesson.** The obvious fear — `derivativesRealized` now
  double-counting the gain — never happened, because `inferCashFlow` reads `Δcash`, never `total_value`, and the
  payout **does** land in brokerage cash (proved from the recorded series: had it not, the 08-30 step's flow would
  have been −$1,008.45, cleared the $750 floor and moved `cumFlow`; it stayed 0 because the flow was
  `1245 − 1008.45 = 236.55`, the COST BASIS). The residual error was ±cost at each end of a round trip,
  telescoping to zero and floor-suppressed at this account's position sizes. **Nothing in the recorded series was
  corrupted** — which is why the fix was made calmly rather than as a repair.
  **The fix is to stop asking the question.** `brokerageEquity` derives the recorded basis from the COMPONENTS
  (`equity_value + options_value + cash`), which equals `total_value − sleeves` when the broker folds them in and
  equals `total_value` when it doesn't — correct under **both** conventions, so the convention never has to be
  re-litigated. The guard that replaced the old one is a real invariant checked against the payload rather than a
  restatement of the model: whatever `total_value` carries beyond the brokerage book must be exactly the sleeves
  we model, and an unexplained residual means an asset bucket the producer has never heard of. It **pushes**
  (`alerts.mjs`, transition-based) rather than only warning into a scheduled run's log — the old one would have
  fired silently on every run for weeks. One-time cost: the perimeter moved, so the first point on the new basis
  books the sleeve carve-out as a FLOW (`basisShift`) — otherwise a −1.07% step that was purely a change of
  measuring stick would compound into a series whose whole purpose is a deposit-immune return.
...
- **Robinhood's `equity_value` is GROSS long market value — the account's equity is `total_value` (v116).**
  ...
  **v119 — the self-directed YTD tile is now the ACCOUNT's year, not a model of it.** The two sides of
  ...
  `data.main.equityHistory` forward through the shared `equityseries.mjs` (from `total_value` until v143, from
  the brokerage book via `brokerageEquity` since — see the prediction-market gotcha), and
...
- **THE DEFENSIVE FLOOR WAS RIGHT FOR A MANDATE THE ACCOUNT NO LONGER HAS (2026-09-08).** Read the entry
  below as the argument FOR a floor — it is a good one, and it is why the mechanism is retained rather than
  deleted. What changed is the objective it was serving. Mandate A makes the account's job "beat SPY over
  rolling 12-month windows", and against that objective a 15% floor is not insurance, it is a 6pp
  structural short of the benchmark's growth names before a single stock is picked (SPY is itself ~9%
  defensive). Measured live: −0.54pp in a fortnight, zero realized protective benefit, against a breaker
  that has never fired in the account's life (deepest drawdown −3.47%). **The generalizable lesson is that
  a control can be individually well-reasoned and still be wrong once the objective moves** — so when a
  mandate changes, re-derive the dials from the objective rather than defending each one on its own
  merits. What replaced it: the drawdown breaker now measures the book against SPY rather than against its
  own peak (so it acts on the model failing, not on the market falling), and the correlation-cluster caps
  — untouched — became the primary downside control. `AG_DEFENSIVE_MIN` and `AG_DIVERSIFIER_MIN` are 0;
  both exposures are still measured and reported.
...
- **A SCHEDULED JOB THAT CORRECTLY DOES NOTHING REPORTS THE SAME "SUCCEEDED" AS ONE THAT WORKED
  (2026-09-02).** The weekly research Routine fires `12 11 * * 1` (Mondays) and its first step is
  `agentic-due.mjs`, which gated at `asOf >= 7d`. But every committed target has been stamped on a
  TUESDAY, so the gap the Monday cron sees is **6 days** and the gate refused every time. On 2026-08-31
  it fired, ran **109 seconds**, printed AGENTIC_NOT_DUE, stopped, and logged
  `ROUTINE_RUN_STATUS_SUCCEEDED` — which is honest, since a clean early exit IS success. **Nothing
  anywhere said the research had skipped a week.** `REFRESH_DAYS` was set to 6 — **and that was still
  wrong, caught 2026-09-03**: the justification ("every target landed on a Tuesday") was false, two had
  landed on Wednesdays and so had the current one, so the 09-07 fire would have seen a 5-day-old target
  and skipped the week again. **No age threshold can work here** — threshold N blocks a whole band of
  weekdays — so the gate is now a **calendar-week test** with `REFRESH_DAYS` demoted to a staleness
  backstop, and `agentic-due.test.mjs` pins every (asOf-weekday, fire-weekday) pair. See the key-files row.
  **Three things generalize.**
  **(a) A cadence gate must be looser than the cadence it gates.** Equality (`>= 7` against a 7-day
  cron) has zero tolerance for the stamp landing an hour late, and the penalty for missing is not an
  hour — it is a WHOLE PERIOD, because the next attempt is a week away.
  **(b) The damage surfaced somewhere else entirely.** A stale target is not itself visible; what showed
  up was the deploy planner waiving MA's and V's entry bands, because zones go advisory past
  `ENTRY_ZONE_STALE_DAYS` (7). That waiver is correct in isolation — a stale zone must not strand cash
  forever — but it was designed assuming the refresh RUNS. When the refresh silently stops, a backstop
  quietly becomes the primary decision-maker. **Check what a safety valve starts deciding when the thing
  it backs up is broken.**
  **(c) `last_run: SUCCEEDED` proves a Routine RAN, never that it DID anything.** Two signals separate
  them and cost nothing to check: an implausibly short `finished_at - fired_at` (109s cannot be a
  60-name research pipeline), and the absence of the artifact the run exists to produce — here, no
  commit to `agentic-target.json` on a research day.
- **A ROUTINE THAT HAS NEVER RUN CAN LOG SUCCEEDED EVERY WEEK — AND THE "BACK AND FORTH" WAS THREE
  CONFIG GAPS, NOT THE MODEL (2026-09-02).** The owner's complaint was that the agentic account needed
  constant permission prompts and chat round trips. Three audits found nothing wrong with the code; all
  three causes were **Routine configuration, which lives server-side and is invisible from this repo**.
  **(a) The weekly research Routine had no connectors and no repo source**, so it had never once produced
  a target — every `agentic-target.json` in git came from an interactive session, and the file was 8 days
  stale, which pushed entry zones past `ENTRY_ZONE_STALE_DAYS` and let today's ticket fill MA and V
  outside their bands. It reported SUCCEEDED weekly throughout. (This is the previous gotcha's lesson
  arriving a second time by a different door: the gate was fixed, the Routine still wasn't running.)
  **(b) The executor Routine was bound to one persistent session since 08-10** — 128M cached tokens and
  ~$74 across ~115 fires that mostly print `EXEC_IDLE` and stop — with no Robinhood tools pre-approved
  and no `PushNotification`, and it had become **stuck in a "needs input" state about a different
  Routine**. A persistent-bound Routine also cannot use completion push notifications at all, so every
  "say what failed" degraded into a chat message someone had to go and read. **(c) The "one-tap confirm"
  the dashboard, the pushes and `AGENTIC.md` all promised did not exist** — the only confirm path was a
  human typing into a Claude session — and the dated pre-authorization block the executor prompt
  references was spent on 08-12 with no template left behind.
  **The tells, all cheap:** a Routine whose `last_run` is SUCCEEDED but whose *artifact* never lands (no
  commit to `agentic-target.json` on a research day); a `finished_at - fired_at` too short for the work;
  a persistent `session_id` repeated across every fire in `list_triggers`; and a promise in the UI copy
  that no code path implements — grep for it.
  **The fixes.** The research Routine's prompt was rewritten in place for a fresh session (with a
  never-fail-silently push rule), a fresh-session executor replacement was created (`claude-opus-5`, push on,
  DISABLED until the owner attaches Robinhood to it in the Routine UI — the API rejects connectors); `producer/agentic-confirm.mjs` makes the confirm a
  real one-command action; `AGENTIC.md` carries an empty PRE-AUTH template; and `SCHEDULING.md` §5 now
  records what each Routine's server-side config must be.
  **Three things generalize.**
  **(i) Connectors and `allowed_tools` are configuration exactly like the prompt — but unlike the prompt
  they cannot be set from a session.** `update_trigger` changes prompt/schedule/name/enabled/model; the
  Routines API in this org **rejects a `connectors` parameter outright**, and `allowed_tools` is
  Routine-UI only. So a session can diagnose this class of failure and cannot fix it. Say which surface
  the fix lives on. (The hourly producer Routine is unchanged and correct, but the same limit applies to
  it: its `allowed_tools` is editable only in the claude.ai Routine UI.)
  **(ii) Never pin MCP tool names in `allowed_tools`.** The full name carries a **session-specific**
  server id — this repo's docs said `mcp__claude_ai_Robinhood__get_portfolio` while a live session sees
  `mcp__1ad8dd47-…__get_portfolio` — so a copied prefix matches nothing and every broker call becomes a
  permission prompt. `preset:default` + the connector attachment is the durable form; `PRODUCER.md` now
  writes tools as `Robinhood · get_portfolio` for the same reason.
  **(iii) A promise in the UI is a spec.** "One-tap confirm" shipped in v96 and was still on the card,
...
- **A ROUTINE BOUND TO A PERSISTENT SESSION IS ONLY AS ALIVE AS THAT SESSION — AND WHEN THE SESSION DIES
  ...
  Routine now has. **Except for one field, caught the same afternoon: `sources`.** The replacement's first
  real run completed the research end to end and was refused on the push — 403, *"not in this session's
  authorized repository set"* — because the git proxy scopes PUSH credentials to the Routine's repo
  `sources`, which `create_trigger` cannot set and which was `[]`. The producer Routine, the one that
  pushes 13×/day, has it only because it was made in the UI ("Select a repository"). A public repo clones
  fine without a source, so nothing upstream of the push can reveal the gap — and the executor carried the
  identical hole and had simply never needed to push (every fire since going live idled). Both must have
  the repo selected in the UI; the prompts now also carry an `add_repo(access:"push")` retry as a
  code-side fallback. (c) **Where a connector is attached is the whole question.** On the ROUTINE (UI,
...
- **THE TWO ACCOUNTS' POSITION ARRAYS CAME BACK TRANSPOSED — AND ONLY THE RUNNING TOTAL WAS PERMANENT
  ...
  **OUTCOME, and the residual left behind.** The 20:42Z run fetched both accounts correctly, and because
  ...
  ≈0.71pp once it compounds — measured 2026-09-01: ••••0741 read −2.86% against a true −2.16%). They
  were **CORRECTED on 2026-09-01 by a one-time migration run inside the PRODUCER**, and the reasoning is
  the part worth keeping. **No ordinary run can shed a corrupted running total:** `appendEquityPoint` reads
  `priorCum` from the point it is REPLACING, so every same-day rebuild inherits the bad value and it is the
  permanent baseline for every future point — which is why this sat open for a day looking like a manual
  chore. But `data.json` is the PRODUCER's own file, rewritten hourly through build-data → emit → validate
  → publish, so a total the pipeline corrupted is the pipeline's to correct, on that same path and under
  the same validation. `repairPriorCumFlow(prior)` ran in `build-data.mjs` immediately after `loadPrior()`,
  BEFORE anything read the series — one call site that corrected both the `appendEquityPoint` path and the
  carry-forward branches. **Prefer this shape to a hand edit for any future data repair:** a hand-edited
  encrypted blob is reviewable by nobody and reproducible by no one, while a migration is tested,
  committed, self-applying and self-disabling (it keyed on the anchor still holding the EXACT bad value to
  the cent, so the first run that applied it made every later run a no-op — firing twice would have
  invented the mirror-image error, and exact equality rather than a tolerance meant a genuine later deposit
  would have shifted the anchor and correctly stopped it). It landed on the 15:35Z run; the agentic series
  went to a flat 7,950.75 and main to a flat 0, verified by decrypting the published snapshot, and the next
  appended point carried `flow=0` so the correction propagated instead of being re-inferred. The module and
  its build-data block were **DELETED the same day** once spent across four runs — it was a migration, not
  a guard, and the root cause is fixed separately below. `git log producer/cumflow-repair.mjs` has it if a
  similar repair is ever needed.
...
- **WIDENING A FUNDING POOL IS NOT THE SAME AS FREEING THE MONEY (2026-08-28).** `agentic-deploy`'s buy
  ...
  **(d) THE INVARIANT'S FIRST REAL CATCH WAS THE FIX ITSELF (2026-09-08).** The re-size path introduced
  above sizes buys against `cash + proceeds` — and then rounded each of ten legs to cents independently,
  which summed **two cents OVER** the pool it had just been sized against ($1,817.31 vs $1,817.29). The
  invariant refused the ticket, correctly: the broker would have rejected the final leg. Two things worth
  keeping. **A fix that computes a budget must also be held to it** — the reconciliation was written to
  stop over-spending and then over-spent by rounding, which is precisely why the check is an INVARIANT
  against reality rather than another figure derived from the same model. And **the failure was two
  cents on $1,817, which is exactly the size that argues for fixing the allocator rather than widening
  the tolerance**: a cent of slack would have hidden this and every future rounding error with it, while
  the real rule ("never size above the pool") is cheap to enforce and needs no tolerance at all.
...
- **A GUARD WITH NO MATERIALITY FLOOR SPENDS REAL MONEY TO SAVE PENNIES — AND THE PARK VEHICLE FEEDS IT
  (2026-09-03).** The wash-sale ledger blocked on ANY realized loss with no minimum, so a **$1.01** VTI
  loss from 09-02 was deferring a **$29.25** VTI target buy until Oct 2. Five more dust entries sat in the
  live ledger the same day (VTI −$0.24, UNH −$1.02, CIFR −$1.11, AAPL −$9.06, GE −$23.96).
  **The structural part: VTI is BOTH the parking vehicle AND a 5% target holding, and nothing anywhere
  excludes the vehicle from the research target** — CLAUDE.md claimed it was "absent from the target by
  design" and it has been IN the target since at least 2026-08-25. Parking is by construction a
  short-duration round trip in a broad-market ETF, so it manufactures small realized losses on its own
  ticker roughly every other release — which then gate that ticker's genuine target allocation. A guard
  was eating its own tail.
  **What the fix is NOT.** Parking itself was never blocked (the park leg isn't washMap-checked), so the
  mechanism kept working; only the target's own buy was gated. And the fix is not "exempt the vehicle" —
  a real VTI loss should still block a real VTI rebuy. It is **materiality**: `WASH_MIN_LOSS` $25.
  **Three things generalize.**
  **(a) Know what a ledger is FOR before deciding how strict it should be.** This one is a pre-trade
  avoidance tool, not a tax record — Robinhood reports within-account wash sales on the 1099-B whatever
  this file says — and a disallowed loss is not lost, it moves to the replacement shares' basis. So the
  cost of ignoring a $1 wash is a timing difference on pennies; the cost of honouring it is a 30-day
  block on a real allocation. Strictness that buys nothing is not conservatism.
  **(b) Filter at the ONE site every consumer reads.** Four surfaces build a washMap from
  `data.agentic.recentLosses` (planner, exec gate, agentic card, self-directed plan). Filtering in
  `lossesFromTrades` gives all four the behaviour at once; filtering per-surface is how this repo keeps
  producing card-vs-executor drift.
  **(c) The floor must test the DAY's total, not each fill, and must not reach the inference path.**
  Five −$20 clips are a −$100 loss and must still block, so a per-trade floor would wave it through. And
  the Railway inference fallback records no `realized` at all — unknown magnitude has to fail safe and
  keep blocking, which falls out for free because the filter keys on a field those entries lack.
  **The same day the owner moved idle cash to a HIGH-YIELD SWEEP, which retired most of the waiting
  ground.** Parking only ever paid for itself by earning beta that cash could not; the repo's own
  measurement already showed it losing (+$0.78 gross on ~$1,000 of turnover against a 0.21% spread, while
  VTI fell 0.22%). `PARK_DATED_REASONS` is now `{wash-sale}` alone — the one window long enough to be
...
- **A SHARED COMPUTATION HAS EVERY CALLER, OR IT IS NOT SHARED — THE MANDATE A VERIFY (2026-09-08).** The
  Plan→Build→Verify split earned its keep on its largest change yet: the build passed its full suite, the
  exec-gate simulation, the privacy audit and a finalize-target drive, and the adversarial verify still
  found **three real defects and one design hole**, none of which any test could see. All four were the
  same shape — a change made correctly at ONE site of something that has SEVERAL.
  **(a) The relative drawdown breaker was wired into the exec gate and not into `build-data.mjs`.** The
  gate computed relative (−5pp trips); the snapshot — which the card, the hand-off prompt and
  `alerts.mjs`'s push all read — computed absolute-only (−20% trips), at a call site whose own comment
  says the two "can never disagree". The executor would have stopped buying with nothing on the phone
  and no push. The build-data test never asserted on the drawdown block at all, which is how it slipped;
  it now pins `benchStale:true`, a flag only reachable when a bench was actually passed.
  **(b) The push text quoted the absolute figure** for a breaker that now trips on the relative one — a
  "−3.2% below its peak" push against a −5pp threshold reads as the breaker being broken.
  **(c) The stale-zone asymmetry was applied in the planner and not in the consumer's mirror**
  (`agenticExecMap`), so on a stale target the card said "entry checks are OFF" while the executor
  deferred above-entry — the card-vs-executor drift the repo's own rules name as worse than no badge.
  **(d) The deposit tranche was inert.** It rationed cash to 50% "per pass", but the executor runs HOURLY
  and `idleDays` advances at midnight, so the remainder was re-rationed every hour: a $2,000 deposit went
  in as $1,000/$500/$250/$250 across four consecutive hours. Fully deployed in ~3 hours; variance
  reduction zero. The build's test ran ONE pass and passed. Replaying the hourly loop is what exposed it.
  **Three things generalize.** (i) **When a computation is shared, grep every call site before the build
  is done** — the bench had to reach `bookDrawdown` at both sites, the asymmetry had to reach both the
  planner and its mirror, and each was "done" after one. (ii) **A test of a mechanism that runs on a
  schedule has to run it on the schedule** — one pass proves the arithmetic, the hourly loop proves the
  mechanism. This is the verify checklist's "simulate the next scheduled run" rule, which the build
  applied to the gate and not to the planner's own new rule. (iii) **The builder's summary is not
  evidence.** Every one of these was described as done, in good faith, in a summary that read as
  complete; the defects lived in the seams between correct pieces, which is where a re-read of the
  summary cannot look and an adversarial pass has to.
- **AN ENCRYPTED FILE COMMITTED ON A SCHEDULE IS A PERMANENT, COMPOUNDING COST (2026-09-08).** The
  producer publishes a ~7MB encrypted `data.json` to a PUBLIC repo about thirteen times a day.
  Encryption is the part that makes this bite: the envelope carries a random salt, so consecutive
  snapshots share no bytes, and git can neither delta nor compress them. **Every run appends a full
  fresh copy to history, forever.** By 2026-09-08 `.git` had reached **1.7GB**, growing ~2.7GB/month,
  and nothing in the repo said so because the working tree is only ever one file.
  **It surfaced as a completely unrelated-looking failure.** The agentic executor's first real test
  fire spent **over ten minutes inside `git fetch`** and never reached the gate before the market
  closed — a Routine that looked hung, on the day its connector was finally attached. The producer
  pays the same tax on all thirteen daily runs (it fires at :35 and commits at :41).
  **Two fixes, at different layers.** (a) The executor's prompt now fetches **`--depth 1`** and is
  told never to fetch unbounded, never `--unshallow`, and never to rebase (a shallow checkout has no
  merge base — on a rejected push it re-does the shallow fetch and re-applies its three files).
  Pushing FROM a shallow clone is routine and safe; it is what `actions/checkout` does by default.
  (b) `histbars.mjs` halves the file itself, which halves the growth for every consumer at once.
  **What was deliberately NOT done: `run.mjs`'s `git fetch origin main` was left unbounded.** Adding
  `--depth 1` there would convert the producer's own repo to shallow, and the producer is the one
  pipeline whose failure takes the whole dashboard down. Saving five minutes a run is not worth that,
  especially now the compaction has halved the bytes anyway. If it is ever revisited, test the
  shallow **push** path first, in isolation.
  **Three things generalize.** (i) **Size is a function of cadence times payload times retention, and
  retention here is infinite by default** — any recurring commit of an incompressible blob needs that
  arithmetic done ONCE, at design time, because it is invisible until it is expensive.
  (ii) **The cost lands on a different component than the one that creates it.** The producer writes
  the history; the executor, which writes nothing, is what broke. Look upstream of a slow agent before
  concluding the agent is wrong. (iii) **91% of a payload can be one key.** Nobody had ever measured
  the snapshot's composition; one `JSON.stringify(...).length` per top-level key found that `hist` was
  91.5% of it and that two of its seven per-bar fields were read by nothing at all. Measure before
  optimizing, and measure the WHOLE thing, not the part you suspect.
  **Still open, and it needs an owner decision:** the existing 1.7GB does not shrink by itself. Only a
  history rewrite (force-push) removes it, which breaks every existing clone and is destructive, so it
  was not done unilaterally. Shallow fetches make the size operationally invisible to the agents, and
  the halved payload slows the growth; GitHub's own soft warning starts around 1GB.

- **THE ANALYZE PAGE READ STALE BARS AGAINST A LIVE PRICE, AND ITS OWN SCORE HAD NEVER BEEN SCORED
  (2026-09-09).** Three Sonnet research passes (an accuracy audit, a coverage study and a 13,613-read
  walk-forward backtest of `analyzeStock` over the snapshot's own history) found the same page failing in
  four independent ways, none visible on a card.
  **(a) Clock mixing.** `azBars`/`azSeries` read `hist.day` raw with no splice and no age gate while
  `price` came from the live quote — the Portfolio page had spliced since v111 and the Plan page had gated
  since v113, and the Analyze page did neither. 142 of 191 series were stale that day. Measured: a series 20
  bars stale flips the DIRECTION of the call **11.2%** of the time, 40 bars **14.9%** — not a lag, a wrong
  answer, because a series that stops stops at its own high. Fixed by `azHist`, ONE reader every az*
  function goes through (splice ≤5d, haircut 4–10d, WITHHOLD past 10d).
  **(b) The label ladder was effectively binary.** `trendUp = price>sma20 && sma20>=sma50` called every
  routine pullback a downtrend (66% of reads), `sma50 = smaN(closes, min(50,len))` was never null so
  `trendUp` was never null and "Oversold Bounce" fired **0 times in 13,613 reads**, and "Hold / Watch" was
  unreachable. A recommendation engine whose middle states cannot occur is a coin with a gauge around it.
  **(c) Stops were built to lose.** `k = max(5%, vol×2.2)` put the MEDIAN stop 12.3% below entry (52% past
  12%), and because tp1 was `price + k×(price−stop)` a looser stop mechanically inflated the target, so
  tp1 hits paid a measured **+0.23R** against a nominal 1.6–1.8R and the whole thing netted ≈0R on a 58.5%
  win rate. Now 2×ATR/swing-low clamped 3–10%, targets from structure, R:R displayed and enforced.
  **(d) Every component of the setup score was NEGATIVELY rank-correlated with forward return** in this
  sample (decile 1 +2.29% fwd10 vs decile 10 −0.33%), and "Buy Opportunity" ran −2.24pp vs SPY across 169
  names. One bull-market YTD sample with overlapping windows, so the SIGN is the finding, not the size —
  and the response was NOT to flip the engine contrarian on nine months of data. It was to make the page
  measure itself: `azEdgeStats` walks every label forward over the whole universe on every snapshot and
  prints what that state has actually been worth, in the card, next to the call. After the rebuild the
  same replay reads Buy 40% / Hold 29% / Downtrend 22% / Extended 10% / Oversold 0.2% — and Buy Opportunity
  STILL trails (−0.68% fwd20, −2.96pp vs SPY) while Hold/Watch and Downtrend lead, which is exactly what
  the strip now tells the reader, with a −10 confidence haircut, instead of asserting an edge nobody checked.
  **Three things generalize.** (i) **A score that has never been scored is a guess** — build the measurement
  into the surface, so it re-runs on every snapshot and cannot go stale the way a one-off study does.
  (ii) **Two readers of the same data must share one reader function**, or the gate one of them has is a
  gate the other lacks; the Portfolio/Plan/Analyze split had three copies of the bar coalescer and only
  two of them were safe. (iii) **The coverage half needed NO owner action** — the hourly Routine's pasted
  prompt defers to `PRODUCER.md`, so the fetch lists were a repo edit (`hist-plan.mjs`), and the
  `SD_BENCH` copy in `analyze-universe.mjs` is pinned to `index.html` by a regex-extraction test, the
...
- **A THRESHOLD THAT WAS FINE BECAME A SILENT OFF-SWITCH WHEN THE DATA SHAPE CHANGED UNDER IT
  ...
  YTD series. **v141 then moved historicals onto 7-bar TAILS** to make a ~230-name bench affordable —
  and 7 < 20, so from that day essentially every batch was skipped, `ivBySym` came back EMPTY, and every
  estimate-path option premium fell through to `ivProxyFor`'s flat **0.55/0.60** default. Nothing failed;
  the Options tab stayed fully populated with confident-looking numbers.
  **What it produced.** The live 2026-09-11 **RCL cash-secured put** card: strike $242, estimated premium
  **$13.05**, income **~$1,305**, **58% annualized**. That premium implies **~68% IV** on a name whose
  realized vol is **23.2% (30d) / 32.2% (60d)**. The honest figures are **$511 and 23%** — the card
  overstated the income of a short-premium trade by **~2.5x**. FER's CSP read **60%** against a true 13%.
  **Four things generalize.**
  **(a) A validity threshold is a claim about the shape of your inputs, and inputs change.** Nothing was
  wrong with `>= 20 bars`, and nothing was wrong with 7-bar tails; the defect lived in the seam, and the
  change that opened it was in a DIFFERENT file for a DIFFERENT reason. When a data shape changes, grep
  for every consumer that asserts something about its size.
  **(b) The data was already there.** `data.hist.day.RCL` carried **173 bars** — the proxy just wasn't
  looking at the snapshot, only at raw/. Third time this exact shape has bitten: the v119 `instrument_id`
  greeks, the 2026-08-30 derivatives settlements, now this. **Before adding a fetch, check whether the
  producer already has the data and is throwing it away.**
  **(c) The fallback was WORSE than no answer, because it was plausible.** A flat 60% default is a real
  number in a real range; it cannot be told apart from a measurement by looking at it. A default that
  silently substitutes for a measurement must be NAMED when it fires — `options-build` now warns with the
  symbols, because a guard that fires silently is indistinguishable from a broken system.
  **(d) The direction of the error follows the SIDE of the trade.** Overstating IV overstates income on a
  SHORT-premium idea (CSP/covered call — it flatters a trade that is actually being underpaid) and
  overstates cost on a LONG one (conservative). There is no single safe direction, which is the argument
  for measuring rather than defaulting. The mirror now applies: realized vol runs a few points UNDER
  implied, so estimates read slightly conservative for short premium and slightly cheap for long. Closing
  that last gap means preferring real IV the book already holds (`ivObserved`, `liveBySym`), not inventing
  an uplift constant.
  The card's own **"Estimate — confirm the live bid in the chain"** line was doing its job the whole time;
  it is not a licence to ship a figure that is 3x out.
...
- **Cost discipline:** historicals are the expensive part. Since v141 `hist-plan.mjs` decides them: a
  7-bar **tail** for every bench name whose series is ≤10 days behind (8 symbols/call — the ≤3 rule is
  about PAYLOAD size, and a tail is ~1/20th of a YTD series), full YTD only for absent/too-stale names
  (≤3/call, capped 30/run) and 5Y monthly capped 15/run — ~25 calls on a steady-state day. They're
  fetched once/day (FETCH_ALL) and carried forward — never re-fetch them on a light run. The live schedule is **hourly** (the "Portfolio dashboard refresh" Routine, `35 * * * *`
...
- **Portfolio (the Self-directed side of the Accounts tab):** All Positions table (sortable) with a **TOTAL footer row** (value, cost, P&L $, P&L %
  ...
  **🏦 By Account** table underneath splits value · unrealized P&L · realized YTD · est. dividends ·
  ...
  floor (`AG_TLH_MIN` $200 / `AG_TLH_PCT` 10% of cost — raised from $75/5% on 2026-09-08; a harvest sells the WHOLE position and the wash guard then blocks the rebuy 30 days, so the old floor was a de-facto −5% hard stop with a forced month out of a name the research still wanted, and ~$19 of tax on a $75 loss does not pay for that) rather than the margin book's `SMALL_THR` $250
...
- **Plan tab is SPLIT BY ACCOUNT (v108) — the same switcher the Accounts tab carries.** `page-picks`
  ...
  **`pf_acct`** key, so the app has ONE account context — but each side only re-renders while its page is
  ...
  a per-name **Status** badge, with the per-name thesis + `drivers` behind `<details>`, and — since v145 —
  the **construction note** behind *"How this target was built"*: an authoritative header strip
  (active/phase-out split, defensive, diversifier — straight off the structured fields), the synthesis's
  prose one claim per line with tickers and figures weighted, then each thing `finalize-target` enforced
  as its own labelled row. Chunked and weighted only; the text is never rewritten),
...
- **Picks (the "🎯 Plan" tab — renamed from "Picks" in v56):** **sortable + sector-filterable** scored candidates table incl. a
  ...
  **Graded in the PRODUCER since v140 (`pickgrade.mjs`), and honest about what it cannot grade.** It read **0% hit rate · 0W·2L · 42 open** because it walked `data.hist.day` on the phone and that series freezes per symbol days after each scan — the true record over the same episodes is **9W·3L / 75%**. Episodes are now graded upstream and each resolved outcome is **frozen into the snapshot**, so the record survives its own prices going stale; an episode whose series does not reach the measurement date reports **"can't be graded"** (a `?` chip, its own amber sentence in the clarifier) and is excluded from BOTH the hit rate and the average, rather than counted as "open"; a resolved pick is measured **to its exit** (a stop-out is its stop, not the rally that followed); the window closes at 60 days into `⌛ EXPIRED`; and every price in the graded detail says **which** price ("exited"/"last close") and **as of when** — it used to print a per-symbol carried-forward quote, up to two months old, with no date at all. **Compact by default (v89):** the card leads with the 3 headline stats (Hit rate · Avg return ·
...
- **Analyze:** per-ticker technical+fundamental breakdown, Recommendation card, Sentiment & Buzz card,
  ...
  **Options Play** links to the Options tab and flags real greeks when you hold live contracts on the
  name; **limited-data** tickers get an explicit "locked until next run" card, and since **v141** a
  **stale-history** ticker (bars >10 days behind the snapshot) gets a ⏳ card and its technical call is
  WITHHELD rather than shaded (4–10 days: scored, confidence ×0.8, amber "bars N days old" badge) — see
  `azHist` and the v141 gotcha. **v141 also rebuilt the call itself:** `azCore` is a PURE function
  (`producer/analyze-core.test.mjs` extracts it from `index.html` and runs it in node) with a three-way
  trend against the 50-day (buffered, so a routine pullback is `neutral` rather than a downtrend), an
  explicit `extended` state (RSI ≥70, >8% over the 20-day, or at the recent high on +15% momentum) that
  reads **Extended — Wait for Pullback**, the oversold check FIRST so it is reachable, stops = the wider
  of 2×ATR and the swing low clamped to 3–10% below price, targets from structure (nearest pivot/recent
  high ≥3% above, then the 52-week high or 2× that distance) with `rr` shown and a BUY downgraded to
  ACCUMULATE with a "targets don't pay for the risk" line under 1.5:1; the Recommendation card carries
  action · why (the two inputs that moved it) · horizon · R:R · "what flips this call" (one price each
  way) · a data-quality line (bars as-of, quote age, fundamentals inputs, trailing-vs-forward P/E); and
  **📏 Measured in this snapshot** (`azEdgeStats`, incremental O(n) walk-forward over EVERY symbol's
  recorded closes, memoized per `generatedAt`) reports what the current label actually did over the next
  10/20 sessions and vs SPY, folding ±5/−10 into confidence only above 30 reads. Colour-coded signals
...
- **Options:** an **exposure roll-up** (net delta · put cash secured · shares capped · open premium at
  ...
  **Cash-Secured-Put income rankers** (annualized yield); Options P&L (closed trades); and a Knowledge
  card. Estimate premiums use a per-symbol realized-vol IV proxy (`optvol.mjs`; horizon-matched 60-bar window, sourced from the raw bars AND the prior snapshot). Research/education only. **Robinhood
