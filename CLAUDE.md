...
| `producer/agentic-due.mjs` | Weekly gate (like `preflight.mjs`) for the agentic research refresh — `AGENTIC_DUE` (exit 0) / `AGENTIC_NOT_DUE` (exit 20), keyed off `agentic-target.json` `asOf` ≥ **6d** (`REFRESH_DAYS`). **6, not 7, and the slack is load-bearing (2026-09-02):** the Routine fires on a weekly MONDAY cron while `asOf` is stamped whenever finalize-target writes — and every committed target so far landed on a TUESDAY, so Monday-minus-previous-Tuesday is 6 days and a `>=7` gate was never satisfied by the very cron meant to satisfy it. On 2026-08-31 the Routine fired, ran 109s, printed NOT_DUE and stopped — logging SUCCEEDED, because stopping cleanly IS success — and nothing anywhere said the research had silently skipped a week. **A weekly job must not be able to skip a week because its own output landed a day late.** Producer step 7 runs the deep research only when DUE. **v93:** the gate is now also tripped EARLY by `agentic-triggers.mjs` (a deposit or a big held-name gap → `refreshResearch`), so research doesn't wait out the week when something material happens. |
...
- **A SCHEDULED JOB THAT CORRECTLY DOES NOTHING REPORTS THE SAME "SUCCEEDED" AS ONE THAT WORKED
  ...
  anywhere said the research had skipped a week.** `REFRESH_DAYS` is now 6.
