...
| `.claude/workflows/agentic-research.js` | Reusable **named workflow** — the deep multi-factor research (momentum/quality/growth/catalyst sleeves + valuation → adversarial verify → synthesis). Pass a fresh `args.universe`; output drives `agentic-target.json` (via `finalize-target.mjs`). **v93:** the synthesis prompt now asks for **correlation-cluster** (megacap-tech ≤48%) + **vol-scaled** weighting; `finalize-target.mjs` re-enforces those caps deterministically so they hold regardless of the model output. |
...
- **Unit + integration tests:** `for t in producer/*.test.mjs; do node "$t"; done` — covers the
  ...
  **trend gate** and the **recent-stop-out cooldown** — `gradePickClose`/`recentStopCooldown`),
  ...
  `agentic-deploy` earnings-blackout/gap-reverify/wash-sale planner — plus its **v96 full-book layer**:
  off-target exits, TLH thresholds + cross-account wash, two-leg T+1 funding, tax netting, the $500 auto
  tier — `agentic-triggers` deposit +
  refresh events, `agentic-ledger` decision grading, **`agentic-pending`** ticket
  ...
  gate, the megacap-tech exclusion), **`finalize-target`** (the deterministic `drivers` attribution), and a full `build-data.mjs` fixture run
  (empty-bars guard, quotes carry-forward, no-picks log guard, social-sidecar reuse, day-move alert,
  **flow sidecar → `data.flow` + per-symbol carry-forward**, **congressional ledger accumulation + dedupe**). CI runs these on every
...
- **🏛️ Flow & Positioning (v95):** a **Plan-page card** (`renderFlowCard`) + an Analyze **🏛️ Flow read chip**
