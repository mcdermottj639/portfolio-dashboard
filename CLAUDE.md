...
- **Versioning:** any change to `index.html`/`sw.js` → bump **both** `APP_VERSION` (in `index.html`
  `boot()`) and `CACHE_VERSION` (in `sw.js`) together. Currently around **v129** (`pf-v129`) — v127 added the self-directed Rebalance Log, v128 moved it to the Accounts tab beside the agentic one, v129 made both logs **accumulate a usable record** (frozen outcome marks + time-based retention, below).
...
- **🧾 Rebalance Log — SELF-DIRECTED (v127, Accounts tab, ••••0741 side, after Flow):** the mirror of the
  ...
  more history never compounds into an answerable question. Decisions that were already past a horizon when
  tracking began are reported as unmeasurable rather than scored on a window nobody watched.
