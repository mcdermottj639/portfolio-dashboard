...
- **Unit + integration tests:** `for t in producer/*.test.mjs; do node "$t"; done` — covers the
  ...
  **trend gate** and the **recent-stop-out cooldown** — `gradePickClose`/`recentStopCooldown`),
  ...
  off-target exits, TLH thresholds + cross-account wash, tax netting, the $500 auto tier, plus the **v98
  limited-margin layer** — same-session proceeds funding and the PDT day-trade guard across exits/harvests/trims — `agentic-triggers` deposit +
