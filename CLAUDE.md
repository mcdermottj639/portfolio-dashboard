...
| `producer/PROPOSAL-hedgefund-gaps.md` | **Build spec (2026-08-24, NOT yet implemented)** for the institutional-gap closures on the agentic account: look-through cluster caps (SPY/VTI composition counted against the 48% megacap-tech cap), a book-level drawdown circuit breaker, regime-aware deployment pacing, sleeve attribution in the Rebalance Log, plus two owner-gated switches (park vehicle → SGOV; `FLOW_WEIGHT` → 0.10). Read it in full before implementing any of those — it fixes constants, function signatures, test cases and the owner-decision defaults. |
