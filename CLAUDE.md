...
- **Versioning:** any change to `index.html`/`sw.js` → bump **both** `APP_VERSION` (in `index.html`
  `boot()`) and `CACHE_VERSION` (in `sw.js`) together. Currently around **v145** (`pf-v145`) — v145 is the
  **CONSTRUCTION NOTE** pass (2026-09-12): `target.method` is the only place the target explains ITSELF —
