...
| `producer/agentic-due.mjs` | Weekly gate (like `preflight.mjs`) for the agentic research refresh — `AGENTIC_DUE` (exit 0) / `AGENTIC_NOT_DUE` (exit 20). **It is a CALENDAR-WEEK test, not an age test (2026-09-03) — because an age test cannot be made to work here, and it ate a week TWICE.** Pure `researchDue(asOf, today)` + `weekStart` (weeks start Monday, matching the cron), unit-tested in `agentic-due.test.mjs` — the module had **no tests at all**, which is how the same defect shipped twice. Due whenever the committed target was not refreshed during the current ET week; `REFRESH_DAYS` (6) survives only as an absolute-staleness backstop. **Why the age gate was unfixable:** the Routine fires `12 11 * * 1` (Mondays) while `asOf` is stamped whenever finalize-target writes, so a threshold N blocks the Monday fire for every target written 1..N−1 days earlier — a whole BAND of weekdays. `>=7` blocked everything but the previous Monday: on 2026-08-31 the Routine fired, ran 109s, printed NOT_DUE and stopped — logging SUCCEEDED, because stopping cleanly IS success — and the target aged to 8d, pushing MA and V past `ENTRY_ZONE_STALE_DAYS` so the planner waived their entry bands. Dropping to **6** was justified by "every committed target landed on a TUESDAY" — **that was false** (2026-08-05, 08-12 and the current 09-02 are WEDNESDAYS), so the 2026-09-07 fire would have seen a 5-day-old target and skipped the week all over again. **Picking a smaller N only moves which weekdays are fatal.** Encode the cadence instead, and err toward running: a day early costs one pass, a miss costs a whole week and is invisible (what surfaces downstream is the planner quietly waiving entry bands). Producer step 7 runs the deep research only when DUE. **v93:** the gate is now also tripped EARLY by `agentic-triggers.mjs` (a deposit or a big held-name gap → `refreshResearch`), so research doesn't wait out the week when something material happens. |
