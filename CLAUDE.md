...
> is where "12-15 names / SPY 15-20% ballast / a 15% defensive floor" lived. It is `meta_mcp` and so IS
> editable via `update_trigger` (do that in the same change as any code edit here, or the change is
> inert on the next run — the 2026-08-25 leaders-bench lesson). The **executor** picks up the relative
