...
- **Versioning:** any change to `index.html`/`sw.js` → bump **both** `APP_VERSION` (in `index.html`
  `boot()`) and `CACHE_VERSION` (in `sw.js`) together. Currently around **v128** (`pf-v128`) — v128 moved the self-directed Rebalance Log from the Plan tab to the Accounts tab (owner's call, same day it shipped) so it mirrors the agentic one; everything else about v127 is unchanged.
...
- **🧾 Rebalance Log — SELF-DIRECTED (v127, Accounts tab, ••••0741 side, after Flow):** the mirror of the
  ...
  surface. **It briefly shipped on the Plan tab** (v127, same day), which read well beside Track Record —
