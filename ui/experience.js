...
(function () {
  ...
  const routes = {
    ...
    technicals: {area:'research', label:'Technicals', tab:'portfolio', section:'technicals'},
    fundamentals: {area:'research', label:'Fundamentals', tab:'portfolio', section:'fundamentals'},
    flow: {area:'research', label:'Flow & positioning', tab:'portfolio', section:'flow'},
  ...
  function renderToday() {
    ...
    const dayLabel=s.partialDay?'Quoted holdings move · partial':'Quoted holdings move';
    const tone=s.day===null?'neutral':s.day>0?'up':s.day<0?'down':'neutral';
    const direction=s.day===null?'—':s.day>0?'↗':s.day<0?'↘':'→';
    ...
    const movers=s.contributors.map(p=>'<div class="ex-mover"><div class="ex-mover-label"><strong>'+esc(p.symbol)+'</strong><span class="ex-'+(p.day>0?'up':p.day<0?'down':'neutral')+'">'+(p.day>0?'↑ ':p.day<0?'↓ ':'→ ')+esc(money(p.day))+'</span></div><div class="ex-diverging" aria-hidden="true"><i class="ex-bar '+(p.day<0?'negative':'positive')+'" style="width:'+Math.abs(p.day)/peak*50+'%"></i></div></div>').join('');
    html+='<div class="ex-brief-hero"><div><div class="ex-label">'+esc(name)+' / account value</div><div class="ex-hero-value">'+money(s.equity)+'</div><div class="ex-hero-meta"><span>'+s.positions.length+' holdings</span><span>Cash '+money(s.cash)+'</span></div>'+jump('performance','Performance & benchmarks')+'</div><div class="ex-move-tile"><div class="ex-label">'+esc(dayLabel)+'</div><div class="ex-daily-value ex-'+tone+'"><span aria-hidden="true">'+direction+'</span> '+money(s.day)+'</div><div class="ex-coverage"><span style="width:'+(s.positions.length?s.dayCoverage/s.positions.length*100:0)+'%"></span></div><p class="ex-muted">'+s.dayCoverage+' / '+s.positions.length+' holdings have previous-close quotes.</p><small class="ex-muted">Stock contributions, not full account P&L.</small></div></div>';
    html+='<div class="ex-grid">'+card('01 / What moved your holdings',movers?movers+'<div class="ex-axis"><span>Detractors</span><span>Contributors</span></div><p class="ex-muted">Top three absolute contributions from captured quotes. Cash flows and option changes are excluded.</p>'+jump('heatmap','Explore the full heatmap'):unavailable('Previous-close quotes are missing.'));
    html+=card('02 / Where your weight sits',largest?'<div class="ex-exposure"><div class="ex-ring" style="--share:'+share+'%" role="img" aria-label="Largest holding represents '+share.toFixed(1)+' percent of priced long holdings"><div><strong>'+share.toFixed(1)+'%</strong><small>priced holdings</small></div></div><div><div class="ex-label">Largest holding</div><div class="ex-number">'+esc(largest.symbol)+'</div><div>'+money(largest.value)+'</div></div></div>'+row('Share of account equity',weight===null?'Unavailable':weight.toFixed(1)+'%')+'<p class="ex-muted">Ring uses priced long holdings only; equity exposure can exceed 100% with margin.</p>'+jump('risk','Risk & allocation'):unavailable('Position valuations are unavailable.'));
    html+=card('03 / Your next stop','<div class="ex-plan-mark" aria-hidden="true">↗</div>'+(s.target?row('Research date',esc(s.target.asOf || 'Not recorded'))+row('Recorded ticket',esc(s.pending?.status || 'No ticket in snapshot'))+jump('decisions','Read the research & sources'):'<h3>Review your Action Center</h3><p class="ex-muted">Cash-raising, redeployment, and Picks, with your existing account rules.</p>')+jump('plan','Open the full plan'));
    html+=card('04 / Behind the numbers','<div class="ex-source-time"><span aria-hidden="true">◷</span><div><strong>'+esc(ageText(s.generatedAt))+'</strong><small>Published snapshot · '+esc(date(s.generatedAt))+'</small></div></div><p class="ex-muted">Snapshot age does not guarantee every quote is current. Holding-level timestamps remain in Analyze.</p>'+jump('activity','Inspect data & routine status'))+'</div>';
