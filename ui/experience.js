...
(function () {
  ...
  function renderToday() {
    ...
    html+='<div class="ex-grid">'+card('Account snapshot','<div class="ex-label">Account value</div><div class="ex-number">'+money(s.equity)+'</div>'+row(dayLabel,esc(money(s.day)),s.dayCoverage+' of '+s.positions.length+' holdings have a previous-close comparison.')+row('Cash',esc(money(s.cash)))+jump('performance','Open performance & benchmarks'));
    html+=card('One exposure to understand',largest?'<div class="ex-label">Largest priced holding</div><div class="ex-number">'+esc(largest.symbol)+'</div>'+row('Position value',esc(money(largest.value)))+row('Share of account equity',weight===null?'Unavailable':weight.toFixed(1)+'%')+'<p class="ex-muted">Position exposure can exceed equity in a margin account. This is a concentration readout, not a sell instruction.</p>'+jump('risk','See risk, allocation & overlap'):unavailable('Position valuations are unavailable.'));
    html+=card('What moved the quoted holdings',s.contributors.length?s.contributors.map(p=>row(p.symbol,esc(money(p.day)))).join('')+'<p class="ex-muted">Largest absolute day contributions from captured quotes. This does not reconcile cash flows, option changes, or the full account return.</p>'+jump('heatmap','Open the holdings heatmap'):unavailable('Previous-close quotes are missing.'),true);
    html+=card('Where the plan stands',s.target?row('Research date',esc(s.target.asOf || 'Not recorded'))+row('Recorded ticket',esc(s.pending?.status || 'No ticket in snapshot'))+jump('decisions','Read the research & sources')+jump('plan','See targets, blocks & next conditions'):'<p>Your existing Action Center retains cash-raising, redeployment, Picks, and account-specific rules.</p>'+jump('plan','Open Plan & Action Center'));
    html+=card('Know the age of the inputs',row('Published snapshot',esc(ageText(s.generatedAt)),date(s.generatedAt))+row('Holding-level quote timestamps','See Analyze','Publication time is not a guarantee that every underlying feed is current.')+jump('activity','Inspect snapshot & routine status'))+'</div>';
