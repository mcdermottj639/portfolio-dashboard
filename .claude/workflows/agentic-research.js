...
//
// THE SAME ARGUMENT APPLIES TO STABILITY (v124). finalize-target.mjs enforces a 15% DEFENSIVE FLOOR
// (staples / utilities / telecom / REITs / managed care / low-vol vehicles, gated on actually trading
// no wilder than a normal large-cap). A universe with no such names in it cannot satisfy that floor no
// matter how good the synthesis is — the floor would report a shortfall every single week. So when you
// assemble args.universe, ALWAYS carry a defensive block through. The composite will rank most of them
// mid-pack (momentum 0.22 + growth 0.22 structurally favour the AI complex over a utility, and that is
// the point of having a floor rather than trusting the score), so they need to be PRESENT to be picked.
...
const U = (args && Array.isArray(args.universe) && args.universe.length) ? args.universe : [
  ...
  // for VALUE was added then, breadth for STABILITY is this). finalize-target.mjs enforces a 15%
  // defensive floor; without these names in the universe that floor can only ever report a shortfall.
