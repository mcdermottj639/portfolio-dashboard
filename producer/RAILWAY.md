...
| **Self-directed recorded equity basis** (Account Performance / YTD tile) | `fetch_rh.py` `portfolio.json` | **KNOWN DIVERGENCE from the Claude path (2026-09-10).** v143 records this account's equity as the BROKERAGE book (`equity_value + options_value + cash`, via `equityseries.brokerageEquity`) so the derivatives sleeves stay outside a series whose `derivativesRealized` term assumes exactly that. `fetch_rh.py` writes no `options_value`, and `brokerageEquity` refuses to read an absent component as 0 (it would silently drop the whole options mark), so a Railway run **falls back to `total_value`** — the pre-v143 behaviour, with any event-contract sleeve inside the recorded figure. Not a regression, but not parity either: build-data warns when it happens. To close it, emit `options_value` alongside `total_value`/`equity_value`/`cash` in `_portfolio()`; the Node tail then needs no change. Until then, avoid alternating producers mid-week on this account — the recorded series would mix two bases. |
