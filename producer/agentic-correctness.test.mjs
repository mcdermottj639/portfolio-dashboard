...
// Simulate the complete sandbox workflow and canonical promotion without network or broker calls.
{
  ...
  const agent=async (_prompt,opts)=>{
    ...
    if(opts.label.startsWith('verify:')) return {ticker:opts.label.slice(7),businessOk:true,recommendation:'buy',confidence:8,supports:true,entryQuality:6,biggestRisk:'test',entryRisk:'test',evidence};
