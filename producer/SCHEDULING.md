...
missing repo source (the 2026-09-09 failure) cannot lose a research run. **Its non-fast-forward recovery is
`checkout -f -B … FETCH_HEAD` + `git checkout <C> -- producer/agentic-target.json`, NOT `reset --soft`** — the first
version of this prompt used `reset --soft FETCH_HEAD`, and on the 2026-09-09 16:21Z run that staged the producer's
PREVIOUS `data.json` as a reversion (a shallow fetch moves HEAD but leaves the index on the old tree); the run's agent
noticed and restored it by hand. The executor prompt always used the checkout form. **This prompt WAS edited in place
